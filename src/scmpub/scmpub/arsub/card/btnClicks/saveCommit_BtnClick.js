 ...
 * @Author: 王龙华 
 * @PageInfo:  保存提交按钮点击事件 
 * @Date: 2018-06-02 15:58:24 
 * @Last Modified by: liulux
 * @Last Modified time: 2021-11-27 13:52:34
 */
...
export default function buttonClick(props, skipCodes, assign, savecallback) {
	skipCodes = skipCodes ? skipCodes : new Array();
	// save_BtnClick.call(this, this.props, skipCodes, commit_BtnClick);
	...
	let flag = props.validatePageToToast([
		{ name: ARSUB_CONST.formId, type: 'form' },
		{ name: ARSUB_CONST.tableId, type: 'cardTable' }
	]);
	if (!flag.allPassed) {
		return;
	}
	...
	arsubVO['skipCodes'] = skipCodes;
	if (assign) {
		arsubVO['assign'] = JSON.stringify(assign);
	}
	props.validateToSave(arsubVO, () => {
		ajax({
			url: ARSUB_CONST.savecommitUrl,
			...
			success: (res) => {
				if (res.data.isResume && res.data.isResume == true) {
					showResumeModal.bind(this)(
						...
						skipCodes,
						res.data,
						buttonClick.bind(this, props, skipCodes, assign, savecallback),
						props
					);
					return;
				} else {
					...
					if (res.success && res.data) {
						...
						if (
							...
							// this.skipCodes = res.data['skipCodes'];
							this.setState({
						...
						// add by huoyzh 云原生适配
						this.skipCodes = [];
						showSuccessInfo(getLangByResId(this, '4006ARSUB-000042')); /* 国际化处理： 保存成功！*/
					}
				}
			}
		});
