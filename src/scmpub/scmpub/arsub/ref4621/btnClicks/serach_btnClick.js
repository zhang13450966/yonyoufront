...
export default function clickSerachBtn() {
	let searchVal = this.props.search.getAllSearchData(REF4621_CONST.searchId);
	if (searchVal === false) {
		return;
	}
	let queryInfo = this.props.search.getQueryInfo(REF4621_CONST.searchId);
	queryInfo.pageInfo = null;

	setDefData(REF4621_CONST.dataSource, REF4621_CONST.searchId, queryInfo);
