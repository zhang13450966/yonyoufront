 ...
 * @Last Modified time: 2021-03-15 15:58:23
 */
...
import { AREA, ROOT, FILED, PAGEID, STATUS } from '../constance';
...
class VehicleTypeList extends Component {
	constructor(props) {
		...
		initLang(this, [ '4001vehicletype' ], 'scmpub', initTemplate.bind(this, this.props));
	}
	...
	componentWillMount() {
		...
		window.onbeforeunload = () => {
			...
			if (status == STATUS.edit) {
				return getLangByResId(this, '4001VEHICLETYPE-000019'); /* 国际化处理： 当前单据未保存，您确认离开此页面？*/
			}
		};
	}
	...
	getOrgInterface = (type) => {
		if (type == 0) {
			return (
				<MainOrgRef
					refName={getLangByResId(this, '4001VEHICLETYPE-000020')} /* 国际化处理： 物流组织*/
					placeholder={getLangByResId(this, '4001VEHICLETYPE-000020')} /* 国际化处理： 物流组织*/
					ref="mainorg"
					refType="grid"
					required={true}
					refCode={'pk_org'}
					...
					onChange={(event) => {
						if (event.refpk) {
							...
							this.state.pk_org = '';
							this.props.button.setButtonDisabled([ 'Add', 'Refresh', 'Edit' ], true);
							this.props.editTable.setTableData(AREA.listTable, { rows: [] });
						}
					}}
				/>
