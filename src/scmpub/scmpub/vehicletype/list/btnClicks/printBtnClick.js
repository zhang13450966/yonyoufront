 ...
 * @Last Modified time: 2020-09-30 10:25:11
 */
...
export function printBtnClick(props, isOutput = false) {
	...
	if (checkeddatas.length == 0) {
		showWarningInfo(getLangByResId(this, '4001VEHICLETYPE-000008')); /* 国际化处理： 请选择要打印的单据!*/
		return;
	}
	let pks = [];
	checkeddatas.forEach((item) => {
		let pk = item.data.values.cvehicletypeid.value;
		pks.push(pk);
	});
	if (!isOutput) {
		print('pdf', URL.print, {
