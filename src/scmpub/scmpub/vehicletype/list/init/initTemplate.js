 ...
 * @Last Modified time: 2021-09-22 15:59:03
 */
			...
			if (data) {
				...
				if (data.button) {
					...
					props.button.setPopContent(
						FILED.Delete,
						getLangByResId(this, '4001VEHICLETYPE-000006')
					); /* 国际化处理： 确认要删除吗？*/


					props.button.setPopContent(
						FILED.StartUse,
						getLangByResId(this, '4001VEHICLETYPE-000023')
					); /* 国际化处理：确定要启用吗? */
					props.button.setPopContent(
						FILED.StopUse,
						getLangByResId(this, '4001VEHICLETYPE-000024')
					); /* 国际化处理：确定要停用吗? */
				...
				if (type == 1) {
					getMainOrgAfter.call(this, AREA.listTable, URL.orgChange, null, PAGEID.pagecodegroup, 'group');
				}
			}
		}
	);
...
function modifierMeta(props, meta) {
	let event = {
		label: getLangByResId(this, '4001VEHICLETYPE-000022') /* 国际化处理： 操作*/ /* 国际化处理： 操作*/,
		attrcode: 'opr',
		itemtype: 'customer',
		fixed: 'right',
		width: '150px',
	...
	// 	if (item.attrcode == FILED.bsealflag) {
	...
	// 				>
	// 					<NCSwitch
	// 						checked={record.values.bsealflag.value}
	// 						onChange={onChange.bind(this, index, record)}
	// 						disabled={this.state.status == STATUS.edit} //编辑态不可用
	// 					/>
	// 				</div>
	// 			);
	// 		};
...
function onChange(index, record) {
	let flag = record.values.bsealflag.value;
	if (flag) {
