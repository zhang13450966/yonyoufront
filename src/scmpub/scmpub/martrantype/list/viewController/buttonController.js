...
export default function (props, pk_org, status) {
	let isBrowse = status === MARTRANTYPE_CONST.BROWSE;
	// 1.设置界面状态
...
function setTableButtonEnable(props, pk_org) {
	...
	// 2.界面上有数据，修改和删除按钮才可用
	let len = 0;
	if (props.editTable.getAllRows(MARTRANTYPE_CONST.TABLEID, true)) {
		len = props.editTable.getAllRows(MARTRANTYPE_CONST.TABLEID, true).length;
	}
...
function setUIState(props,status) {
	props.editTable.setStatus(MARTRANTYPE_CONST.TABLEID, status);
}