 ...
 * @Last Modified time: 2020-09-30 10:11:23
 */
...
export function startuseBtnClick(index, record) {
	...
	if (type == 0) {
		if (!record.values.pk_org_v.value) {
			showErrorInfo(getLangByResId(this, '4001DRIVER-000010')); /* 国际化处理： 组织节点不启用集团数据*/
			return;
		}
	}
	...
	ajax({
		...
		success: (res) => {
			if (res.success) {
				...
				showSuccessInfo(getLangByResId(this, '4001DRIVER-000013')); /* 国际化处理： 启用成功!*/
	...
	// 执行操作提示
	// showWarningDialog(getLangByResId(this, '4001DRIVER-000011'), getLangByResId(this, '4001DRIVER-000012'), {
	// 	/* 国际化处理： 启用,确定要启用吗？*/
	// 	beSureBtnClick: () => {
	// 		let pk = record.values.cdriverid.value;
	// 		let data = { id: [ pk ], pagecode: pageid };
