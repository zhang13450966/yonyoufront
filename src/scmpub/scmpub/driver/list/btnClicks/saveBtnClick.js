 ...
 * @Last Modified time: 2021-11-24 14:34:54
 */
...
export function SaveBtnClick() {
	this.props.editTable.filterEmptyRows(AREA.listTable, [ 'pk_org', 'org_id' ]);
			...
			success: (res) => {
				...
				if (res && res.data) {
					if (res.data[AREA.listTable]) {
						...
						//年龄字段补充
						for (let i = 0; i < res.data.listhead.rows.length; i++) {
							if (rows[i].values.age && rows[i].values.age.value)
								res.data.listhead.rows[i].values.age = {
									value: rows[i].values.age.value
								};
						}
				...
				showSuccessInfo(getLangByResId(this, '4001DRIVER-000009')); /* 国际化处理： 保存成功!*/
