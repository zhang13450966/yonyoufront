 ...
 * @Last Modified time: 2020-09-11 14:25:50
 */
export default function afterEvent(props, moduleId, key, value, changedrows, i) {
	//包装分类参照编辑后放入名称
	if (key === 'pk_packsort') {
		if (value.refcode) {
			props.editTable.setValByKeyAndIndex(moduleId, i, 'pk_packsort.vpackingtypename', {
				value: value.refname,
				display: value.refname
			});
		} else {
			clearValue(this.props, moduleId, i, [ 'pk_packsort.vpackingtypename' ]);
		}
	} else if (key === 'nlength' || key === 'nwidth' || key === 'nheight') {
		if (value == '') {
