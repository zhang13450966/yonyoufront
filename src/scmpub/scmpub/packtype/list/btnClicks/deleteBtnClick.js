...
export default function(props, record, index, isLine) {
	...
	if (isLine) {
		...
		if (rows.length <= 0) {
			showWarningInfo(getLangByResId(this, '4001PACKTYPE-000003')); /* 国际化处理： 请选择要删除的数据*/
			return;
		}
		let indexes = []; //选中行号
		//删行
		...
		if (status == UISTATE.browse) {
			...
			//浏览态删除
			showWarningDialog(
				getLangByResId(this, '4001PACKTYPE-000004'),
				getLangByResId(this, '4001PACKTYPE-000005'),
				{
					/* 国际化处理： 删除,确定要删除所选数据吗？*/
	...
	ajax({
		...
		success: (res) => {
			if (res && res.success) {
				...
				showSuccessInfo(getLangByResId(this, '4001PACKTYPE-000006')); /* 国际化处理： 删除成功！*/
