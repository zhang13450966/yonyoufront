...
export default function(props) {
	//过滤空行
	props.editTable.filterEmptyRows(PAGEAREA.list, [ 'pk_org', 'pk_group', 'pseudocolumn' ]);
	let rows = props.editTable.getAllRows(PAGEAREA.list);
	if (!rows || rows.length == 0) {
		viewController.call(this, props);
		return;
	}
	let flag = props.editTable.checkRequired(PAGEAREA.list, rows);
	if (flag) {
		let data = {
			pageid: PAGECODE,
			model: {
			...
			ajax({
				...
				success: (res) => {
					if (res && res.success) {
						...
						showSuccessInfo(getLangByResId(this, '4001PACKTYPE-000008')); /* 国际化处理： 保存成功！*/
						//更新页面状态
						viewController.call(this, props);
					}
				}
