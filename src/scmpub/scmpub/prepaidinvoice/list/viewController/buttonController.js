...
function setButtonState() {
	...
	// 1.获取列表勾选的数据
	let selrows = this.props.table.getCheckedRows(PREPAIDINVOICE_CONST.formId);
	...
	this.props.button.setPopContent(
		BUTTON.delete,
		getLangByResId(this, '4006PREPAIDINVOICE-000015')
	); /* 国际化处理： 确定要删除吗？*/
	//如果有查询缓存则刷新按钮可用
	if (getDefData(PREPAIDINVOICE_CONST.PrepaidinvoiceCacheKey, PREPAIDINVOICE_CONST.searchId)) {
...
function getDisabledBtnArray(props) {
	...
	// 单据状态：0=自由，1=审批中，2=审批未通过，3=审批通过
