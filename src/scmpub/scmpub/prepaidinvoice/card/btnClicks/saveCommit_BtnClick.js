 ...
 * @Last Modified time: 2021-07-22 10:23:12
 */
...
export default function buttonClick(props, skipCodes, assign) {
	// save_BtnClick.call(this, this.props, skipCodes, commit_BtnClick);
	{
		...
		if (assign) {
			prepaidinvoiceVO['assign'] = JSON.stringify(assign);
		}
		//运输单拉单是回写ID
		...
		props.validateToSave(prepaidinvoiceVO, () => {
			ajax({
				url: PREPAIDINVOICE_CONST.saveCommitUrl,
				...
				success: (res) => {
					...
					if (res.success && res.data) {
						...
						if (bill.head && bill.body) {
							hid =
								bill.head[PREPAIDINVOICE_CONST.formId].rows[0].values[PrepaidinvoiceHeadItem.hid].value;
						...
						showSuccessInfo(getLangByResId(this, '4006PREPAIDINVOICE-000037'));
					}
				}
			});
		});
