...
export default function clickSerachBtn() {
	let searchVal = this.props.search.getAllSearchData(REF4804_CONST.searchId);
	if (searchVal === false) {
		return;
	}
	let queryInfo = this.props.search.getQueryInfo(REF4804_CONST.searchId);
	queryInfo.pageInfo = null;

	setDefData(REF4804_CONST.dataSource, REF4804_CONST.searchId, queryInfo);
