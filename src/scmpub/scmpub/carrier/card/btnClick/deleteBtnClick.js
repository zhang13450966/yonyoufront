 ...
 * @Last Modified time: 2020-11-25 09:03:02
 */
...
function deleteFunction(params) {
	let pk_org = params.props.form.getFormItemsValue(AREA.card_head, HEADFILED.pk_org).value;
	let pk_group = params.props.form.getFormItemsValue(AREA.card_head, HEADFILED.pk_group).value;
	if (pk_org == pk_group) {
	...
	ajax({
		...
		success: (res) => {
			if (res.success) {
				...
				toast({
					color: 'warning',
					content: getLangByResId(this, '4001CARRIER-000007') /* 国际化处理： 删除失败！*/
				});
			}
		}
	});
