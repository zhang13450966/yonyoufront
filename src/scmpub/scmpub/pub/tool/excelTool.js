 ...
 * @Author: qiaobb 
 * @PageInfo: 导出工具类  
 * @Date: 2018-07-05 09:54:06 
 * @Last Modified by: chenggangk
 * @Last Modified time: 2022-05-13 11:00:28
 */
/**
 * 
 * @param {*} props      
 ...
 * @param {*} titleMeta 
 */
function constructExcelData(props, headArea, bodyArea, titleMeta, tableType, formatField) {
	let meta = props.meta.getMeta();
	let headData = null;
	if (headArea != null) {
		headData = props.form.getAllFormValue(headArea);
	}
	let bodyData = null;
	if (tableType == 0) {
					...
					for (let i = 0; i < bodyTitleAry.length; i++) {
						...
						if (codeValue == undefined) {
							...
							if (Object.keys(codeValue).length === 0) {
								...
								if (codeValue.hasOwnProperty('display')) {
									if (
										(codeValue.display && formatField == undefined) ||
										(codeValue.display && formatField && formatField.includes(bodyTitleAry[i].code))
									) {
										excelcell.value = codeValue.display;
									} else {
										excelcell.value = codeValue.value;
