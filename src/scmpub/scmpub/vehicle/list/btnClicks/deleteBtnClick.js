 ...
 * @Last Modified time: 2020-10-20 16:31:48
 */
...
export function deleteBtnClick() {
	...
	for (let i = 0; i < checkeddata.length; i++) {
		...
		if (type == 0) {
			if (checkeddata[i].data.values.pk_group.value == checkeddata[i].data.values.pk_org.value) {
				showErrorInfo(getLangByResId(this, '4001VEHICLE-000002')); /* 国际化处理： 组织节点不能删除集团级节点数据*/
				return;
			}
		}
	}
	...
	showWarningDialog(getLangByResId(this, '4001VEHICLE-000003'), getLangByResId(this, '4001VEHICLE-000004'), {
		...
		beSureBtnClick: () => {
			...
			let rows = this.props.editTable.getCheckedRows(AREA.listTable);
			rows.map((row) => {
				pk.push(row.data.values.cvehicleid.value);
			});
