 ...
 * @Last Modified time: 2020-09-30 09:56:08
 */
...
export function printBtnClick(props, isOutput = false) {
	let checkeddatas = props.table.getCheckedRows(AREA.listTable);
	if (checkeddatas.length == 0) {
		showWarningInfo(getLangByResId(this, '4001CARRIERGROUP-000020')); /* 国际化处理： 请选择要打印的单据!*/
		return;
	}
	let pks = [];
	checkeddatas.forEach((item) => {
		let pk = item.data.values.ccarrierid.value;
		pks.push(pk);
	});
	if (!isOutput) {
		print('pdf', URL.print, {
