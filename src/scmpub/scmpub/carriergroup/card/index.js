 ...
 * @Last Modified time: 2021-02-21 14:23:04
 */
...
const { NCDiv } = base;
class CarrierGroupCard extends Component {
	constructor(props) {
		super(props);
	...
	componentWillMount() {
		...
		window.onbeforeunload = () => {
			...
			if (status == STATUS.edit || status == STATUS.add) {
				return getLangByResId(
					this,
					'4001CARRIERGROUP-000015'
				); /* 国际化处理： 当前单据未保存，您确认离开此页面？*/ /* 国际化处理： 当前单据未保存，您确认离开此页面？*/
			}
		};
	}
	// 渲染页面后，执行的方法
...
export default CarrierGroupCard;
