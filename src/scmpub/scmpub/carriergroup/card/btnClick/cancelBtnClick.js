 ...
 * @Last Modified time: 2021-06-10 09:26:54
 */
			...
			beSureBtnClick: () => {
				...
				if (from == 'list') {
					...
					if (ccarrierid && 'null' !== ccarrierid) {
						//获取缓冲中的数据
						// let cardData = getCacheDataByPk(props, CARRIERDATASOURCE.carrierdatasource, ccarrierid);
						// if (cardData && cardData.carrier) {
						// 	this.props.form.setAllFormValue({
						// 		[AREA.card_head]: cardData.carrier[AREA.listTable]
						// 	});
						// }
...
function getCardData(props, pk, flag) {
	let data = { id: pk, pagecode: PAGEID.cardpagecodegroup };
	ajax({
		url: URL.cardQuery,
		data: data,
		method: 'post',
