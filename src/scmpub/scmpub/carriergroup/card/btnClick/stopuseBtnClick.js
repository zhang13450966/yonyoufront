/*
 * @Author: zhaochyu
 * @PageInfo: 停用
 * @Date: 2020-02-10 18:46:54
 * @Last Modified by: zhangflr
 * @Last Modified time: 2020-12-29 14:02:54
 */
import { ajax } from 'nc-lightapp-front';
import { AREA, PAGEID, URL, HEADFILED, STATUS, CARRIERDATASOURCE } from '../../constance';
import { showWarningDialog, showSuccessInfo } from '../../../pub/tool/messageUtil';
import { getLangByResId } from '../../../../../scmpub/scmpub/pub/tool/multiLangUtil';
import { updateCacheData, getCacheDataByPk } from '../../../../../scmpub/scmpub/pub/cache';
export default function stopuseBtnClick(props) {
	// 执行删除操作提示
	showWarningDialog(
		getLangByResId(this, '4001CARRIERGROUP-000011'),
		getLangByResId(this, '4001CARRIERGROUP-000012'),
		{
			/* 国际化处理： 停用,确定要停用吗？*/
			beSureBtnClick: () => {
				this.props.form.setFormItemsValue(AREA.card_head, {
					[HEADFILED.bsealflag]: {
						value: true,
						display: getLangByResId(this, '4001CARRIERGROUP-000013') /* 国际化处理： 是*/
					}
				});
				let data = this.props.createMasterChildDataSimple(
					PAGEID.cardpagecodegroup,
					AREA.card_head,
					AREA.driver
				);
				let savedata = data.head;
				savedata.pageid = PAGEID.cardpagecodegroup;
				savedata.templetid = data.templetid;
				savedata.card_head.rows[0].status = '1';
				ajax({
					method: 'post',
					data: savedata,
					url: URL.carduse,
					success: (res) => {
						if (res.data && res.data.card_head) {
							this.props.form.setAllFormValue({
								[AREA.card_head]: res.data.card_head
							});
							updateCache.call(
								this,
								res.data.card_head.rows[0].values.ccarrierid.value,
								res.data.card_head.rows[0].values.bsealflag
							);
							showSuccessInfo(getLangByResId(this, '4001CARRIERGROUP-000014')); /* 国际化处理： 停用成功!*/
							this.toggleShow(STATUS.browse);
						}
					}
				});
			}
		}
	);
}

function updateCache(pk, bsealflag) {
	let cardData = getCacheDataByPk(this.props, CARRIERDATASOURCE.carrierdatasource, pk);
	if (cardData && cardData.carrier && bsealflag) {
		cardData.carrier.list_head.rows[0].values.bsealflag = bsealflag;
	}
	updateCacheData(
		this.props,
		HEADFILED.ccarrierid,
		pk,
		cardData,
		AREA.card_head,
		CARRIERDATASOURCE.carrierdatasource
	);
}
