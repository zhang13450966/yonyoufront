...
export default class ButtonAction {
    ...
    printTable = () => {
        const {props} = this.comp;
        const {trip, editTable, meta} = props;

        let tableWrapper = document.getElementById('mainTable');

        this.print(tableWrapper, {
            title: trip.language['gx6008-000067'], // 录用办理
            maker: trip.language['gx6008-000068'], // 制作者
            date: trip.language['gx6008-000069'], // 制作日期
            tableInfo: {
    ...
    fileManager = (selectedRows) => {
        ...
        let fileManagerBillId = '';
        if (selectedRows) {
            fileManagerBillId = selectedRows[0].values['bill_code'].value;
        } else {
            fileManagerBillId = form.getFormItemsValue('trip_approve_card', 'pk_trip').value;
        }
        dispatch({
            type: 'trip/update',
            payload: {
