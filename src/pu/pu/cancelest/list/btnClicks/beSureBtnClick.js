 ...
 * @Author: zhangshqb 
 * @PageInfo: 取消费用暂估  
 * @Date: 2018-06-26 11:30:43 
 * @Last Modified by: raoczh
 * @Last Modified time: 2020-12-28 15:14:05
 */
...
export default function beSureBtnClick(props) {
	...
	ajax({
		url: urlpath,
		data: { grids: condata, onlyCancelFee: this.onlyCancelFee, pagecode: PAGECODE.pagecode },
		success: (res) => {
			let { success, data } = res;
			if (success) {
				// this.props.editTable.delRowsByIndex(PAGECODE.childTableId, indexs);
				this.props.editTable.setStatus(PAGECODE.childTableId, 'browse');
				this.props.editTable.setStatus(PAGECODE.tableId, 'browse');
				let checkdata = this.props.editTable.getCheckedRows(PAGECODE.tableId);
				...
				toast({
					content: getLangByResId(this, '4004CANCELEST-000000'),
					color: 'success',
					position: 'topRight'
				}); /* 国际化处理： 取消费用暂估成功*/
			}
