 ...
 * @Last Modified time: 2021-07-21 14:26:33
 */
...
export default function clickBtn(props) {
	let head = this.props.form.getAllFormValue(BUYINGREQ_CARD.formId);
	let body = this.props.cardTable.getCheckedRows(BUYINGREQ_CARD.tableId);
	body = body.map((item) => item.data);
	if (body.length == 0) {
		body = this.props.cardTable.getAllRows(BUYINGREQ_CARD.tableId);
	}
	//需要知道传的是第几行的数据
	let rowAndrowid = new Map();
	let indexToRowMap = new Map();
	let allrows = this.props.cardTable.getVisibleRows(BUYINGREQ_CARD.tableId);
	...
	body.forEach((item) => {
		...
		item.values.cbodywarehouseid = item.values.pk_reqstor;
	});
	let onHandRows = processBeforeOnHand('20', head, { rows: body }, [ 'pk_org', 'pk_reqstor' ], indexToRowMap);
	this.setState({
		headRows: onHandRows
	});
