...
class DistributeByMoneyAction {
	...
	doAction(page) {
		...
		let data = {
			invoices: DistributeByMoneyAction.isEmpty(invoice)
				? null
				: {
			...
			discounts: DistributeByMoneyAction.isEmpty(discount)
				? null
				: {
						pageid: PAGECODE.list,
						model: {
			...
			fees: DistributeByMoneyAction.isEmpty(fee)
				? null
				: {
						pageid: PAGECODE.list,
						model: {
			...
			stocks: DistributeByMoneyAction.isEmpty(stock)
				? null
				: {
						pageid: PAGECODE.list,
						model: {
		...
		ajax({
			method: 'post',
			url: URL.invoiceDistributeByMny,
			data: data,
			success: (res) => {
				if (res && res.data && res.data[page.state.matchcode]) {
					...
					DistributeByMoneyAction.refreshHandle(page);
...
export default DistributeByMoneyAction;
