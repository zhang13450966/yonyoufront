...
function showMatch() {
	...
	for (let index = 0; index < rowCount; index++) {
		...
		if (binvoice) {
			...
			this.props.editTable.setEditableRowKeyByIndex(this.state.matchcode, index, 'ncurinvoicesettlenum', true);
			let canStlMny = canStlMnyCol.value[index];
		...
		if (settleType != SETTLTTYPE.FEE) {
			if (bstock) {
				...
				this.props.editTable.setEditableRowKeyByIndex(this.state.matchcode, index, 'ncurstocksettlenum', true);
				...
				// if (canedit.length > 0) {
				// 	this.props.editTable.setEditableRowKeyByIndex(this.state.matchcode, index, canedit, true);
				// }
			}
		}
	...
	// if (settleType != SETTLTTYPE.FEE) {
	// 	for (let index = 0; index < rowCount; index++) {
	// 		let bstock = (this.props.editTable.getValByKeyAndIndex(this.state.matchcode, index, 'bstock') || {}).value;
	// 		if (bstock) {
	// 			this.props.editTable.setEditableRowKeyByIndex(this.state.matchcode, index, 'ncurstocksettlenum', true);
	// 			if (settleType == SETTLTTYPE.WITHOUT_INVOICE) {
	// 				this.props.editTable.setEditableRowKeyByIndex(
...
export default function(props, moduleId, key, value, changedrows, index, record) {
	let event = props.createGridAfterEventData(PAGECODE.vim, this.state.matchcode, moduleId, key, changedrows);
