...
export default function(val) {
	if (this.state.current == 'invoice') {
		...
		if (pk_org && pk_org.value.firstvalue) {
			...
			//toast({ content: '财务组织不能为空', color: 'warning' });
			return;
		}
			...
			success: (res) => {
				...
				if (res.data && res.data.gridMap.all) {
					...
					if (res.data.count > 500) {
						toast({content: getLangByResId(this, '4004SETTLEMENT-000006'), color: 'warning'});/* 国际化处理： 查询结果大于1000行，请缩小条件再试*/
					} else {
					...
					// 根据模块 设置可编辑性。
					//_this.props.editTable.setStatus(AREA.list, 'edit');
				} else {
					this.state.invoice = null;
					this.state.discount = null;
					this.state.fee = null;
		...
		let pk_org = this.props.search.getSearchValByField(AREA.stockInSearchArea, 'pk_stockps_b.pk_financeorg');
