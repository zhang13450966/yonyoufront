 ...
 * @Author: zhaochyu
 * @PageInfo: 到货计划查询
 * @Date: 2019-03-25 10:43:33
 * @Last Modified by: raoczh
 * @Last Modified time: 2020-12-28 15:18:21
 */
...
export default class ReceivePlansQuery extends Component {
	constructor(props) {
		super(props);
	}
	/**
	...
	disposeSearch(meta, props) {
		...
		items.map((item) => {
			...
			if (code == FIELD.pk_supplierAreac || code == FIELD.pk_marbasclass || code == FIELD.cprojectid) {
				item.isShowUnit = true;
				item.isMultiSelectedEnabled = true;
				disposeFilter(props, FIELD.pk_org, item);
			} else if (code == FIELD.billmaker) {
				item.isMultiSelectedEnabled = true;
			} else if (code == FIELD.pk_recvstordoc) {
				item.isShowUnit = true;
				item.isMultiSelectedEnabled = true;
				item.queryCondition = () => {
					let data = getFieldValue2Arr(props, FIELD.pk_arrvstoorg);
	...
	onAfterEvent(props, searchId, field, val) {
		...
		} else if (field == FIELD.pk_arrvstoorg) {
			multiCorpRefHandler(props, val, searchId, [ FIELD.pk_recvstordoc ]);
		}
	}
	render() {
		return (
...
ReactDOM.render(<ReceivePlansQuery />, document.getElementById('app'));
