 ...
 * @Author: zhaochyu
 * @PageInfo: 采购发票明细查询
 * @Date: 2018-06-27 16:51:09
 * @Last Modified by: raoczh
 * @Last Modified time: 2020-12-28 15:18:05
 */
...
export default class PuInvoiceQuery extends Component {
	constructor(props) {
		super(props);
	}
	/**
   ...
   */
	disposeSearch(meta, props) {
		...
		items.map((item) => {
			...
			if (
				...
				code == FIELD.pk_paytosupplier ||
				code == FIELD.pk_payterm ||
			...
			} else if (code == FIELD.pk_bizpsn || code == FIELD.pk_dept) {
				item.isShowUnit = true;
				item.queryCondition = () => {
			...
			} else if (code == FIELD.vtrantypecode) {
				item.queryCondition = () => {
					return { parentbilltype: '25' };
				};
	...
	onAfterEvent(props, searchId, field, val) {
		...
		if (field == FIELD.pk_org) {
			...
			multiCorpRefHandler(props, val, searchId, [ FIELD.pk_stordoc ]);
		} else if (field == FIELD.pk_purchaseorg) {
			multiCorpRefHandler(props, val, searchId, [ FIELD.pk_bizpsn, FIELD.pk_dept ]);
		}
	}
	render() {
		return (
...
ReactDOM.render(<PuInvoiceQuery />, document.getElementById('app'));
