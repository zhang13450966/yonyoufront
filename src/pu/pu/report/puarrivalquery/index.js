...
export default class PuArrivalQuery extends Component {
	constructor(props) {
		super(props);
	...
	componentWillMount() {
		...
		if (params && params.length > 0) {
			params.forEach((param) => {
				if (param.field == 'groupcondition') {
					param.value.firstvalue = '1';
				}
			});
			let conditions = {
				logic: 'and',
   ...
   */
	...
	setConnectionSearch(transSaveObject, obj, data, props, url, urlParam, sessionKey) {
		...
		if (obj.key == 'billDrill') {
			...
			let condit = data.querycondition.conditions.filter((condition) => condition.field == 'groupcondition');
			if (condit && condit instanceof Array && condit[0].value.firstvalue == '1') {
			...
			ajax({
				...
				success: (res) => {
					if (res.success) {
						...
						conds.push({
							field: 'po_arriveorder.vbillcode',
						...
						conds.push({
							oprtype: '=',
							datatype: '204',
							field: 'po_arriveorder_b.pk_srcmaterial.code',
...
ReactDOM.render(<PuArrivalQuery />, document.getElementById('app'));
