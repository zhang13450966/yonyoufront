/*
 * @Author: hufei 
 * @PageInfo: 交易类型管理扩展 适用于 请购单
 * @Date: 2018-07-06 14:17:31 
 * @Last Modified by: hufei
 * @Last Modified time: 2018-12-17 10:47:41
 */
...
import '../index.less';
let template = [ { itemtype: 'switch', label: '4004transfer-000000', code: 'bneedarrange' } ]; /* 国际化处理： 需要请购安排*/

export default class BuyrtrantypeExt extends Component {
	constructor() {
		super();
		this.state = {
			fields: {
	...
	queryData = (billID) => {
		...
		if (!billID) return;
		ajax({
			url: '/nccloud/pu/buyingreq/exttranstypequeryaction.do',
			data: {
			...
			success: (res) => {
				...
				if (success && data) {
					...
					this.setState({
						fields: {
							bneedarrange: data.bneedarrange,
							ts: data.ts
						}
					});
				} else {
...
ReactDOM.render(<BuyrtrantypeExt />, document.querySelector('#transtypebusi'));
