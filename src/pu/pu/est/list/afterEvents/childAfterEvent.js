 ...
 * @Last Modified time: 2021-06-30 14:46:58
 */
...
export default function(props, moduleId, key, value, changedrows, index, record) {
	...
	let body = { po_stockps_fee: { areacode: 'po_stockps_fee', rows: bodyrows } };
	...
	if (headrows.record.values.onebillselect) {
		isonbillselect = headrows.record.values.onebillselect.value;
	}
	// let headrows = props.cardTable.getCheckedRows(PAGECODE.tableId, PAGECODE.childTableId)[0].data.values;
	let head = { po_stockps: { areacode: 'po_stockps', rows: [ { values: headrows.record.values } ] } };
	//编辑后事件：
	let data = {
		itemKey: key,
		body: body,
	...
	ajax({
		...
		success: (res) => {
			if (res.data) {
				let resdata = res.data.body.po_stockps_fee.rows[index];
				// let redstockps = res.data.body.po_stockps_fee.rows[0];
				this.feeItems[resdata.values.pk_stockps_b.value].po_stockps_fee.rows[index] = resdata;

				let resstockpsdata = res.data.head.po_stockps.rows[0];
				if (!resstockpsdata.values.onebillselect) {
					resstockpsdata.values.onebillselect = {};
				}
				if (!isonbillselect) {
					resstockpsdata.values.onebillselect.display = false;
					resstockpsdata.values.onebillselect.isEdit = false;
					resstockpsdata.values.onebillselect.value = false;
				} else {
					resstockpsdata.values.onebillselect.display = true;
					resstockpsdata.values.onebillselect.isEdit = false;
					resstockpsdata.values.onebillselect.value = true;
				}
