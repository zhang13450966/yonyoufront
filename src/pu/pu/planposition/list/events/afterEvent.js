/*
 * @Author: yechd5 
 * @PageInfo: 计划岗物料设置编辑后事件处理 
 * @Date: 2018-05-10 09:40:40 
 * @Last Modified by: yechd5
 * @Last Modified time: 2018-11-17 10:40:48
 */

import { ajax } from 'nc-lightapp-front';
import { PAGEAREA, APP } from '../const';
...
function afterEdit(props, req, editIndex, key) {
	ajax({
		url: PLANPOSITION_CONST.AFTEREVENTURL,
		data: req,
		success: (res) => {
			if (res.data) {
				let newRows = res.data.grid[PLANPOSITION_CONST.TABLEID].rows;
				let updateArray = [];
				for (let j = 0; j < newRows.length; j++) {
					let row = newRows[j];
					let obj = { index: editIndex + j, data: row };
					if (j != 0) {
						props.editTable.addRow(PLANPOSITION_CONST.TABLEID, editIndex + 1, false);
					}
					updateArray.push(obj);
				}
				if (updateArray.length > 0) {
					props.editTable.updateDataByIndexs(PLANPOSITION_CONST.TABLEID, updateArray, false, true);
					clearItemValByKey.bind(this)(props, key, editIndex);
				}

				// let array = [];
				// for (let k = 0; k < res.data.grid[PLANPOSITION_CONST.TABLEID].rows.length; k++) {
				// 	array.push({
				// 		index: k,
				// 		data: {
				// 			status: res.data.grid[PLANPOSITION_CONST.TABLEID].rows[k].status,
				// 			values: res.data.grid[PLANPOSITION_CONST.TABLEID].rows[k].values
				// 		}
				// 	});
				// }
				// for (let z = 0; z < res.data.grid[PLANPOSITION_CONST.TABLEID].rows.length; z++) {
				// 	props.editTable.setRowStatus(PLANPOSITION_CONST.TABLEID, z, array[z].data.status);
				// }
			}
		}
	});
...
function clearItemValByKey(props, key, index){
	...
	if (key === 'pk_srcmaterial') {
		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'pk_marpuclass', empty);
		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'pk_marpuclass.name', empty);
		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'marpuclass_code', empty);

		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'pk_marbasclass', empty);
		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'pk_marbasclass.name', empty);
		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'marbasclass_code', empty);
	} else if(key === 'pk_marbasclass'){
		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'pk_marpuclass', empty);
		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'pk_marpuclass.name', empty);
		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'marpuclass_code', empty);

		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'pk_material', empty);
		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'material_code', empty);
		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'pk_srcmaterial', empty);
		props.editTable.setValByKeyAndIndex(PLANPOSITION_CONST.TABLEID, index, 'pk_srcmaterial.name', empty);
...
function autoAddLine(props, index, moduleId) {
	let allRows = props.editTable.getAllRows(PLANPOSITION_CONST.TABLEID, true);
	let visiablerows = [];
	...
	if (index == len - 1) {
		...
		props.editTable.setValByKeyAndIndex(moduleId, index + 1, 'fpositiontype', { value: '0', display: '0' });
	}
}
