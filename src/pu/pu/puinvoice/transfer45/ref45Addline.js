 ...
 * @Last Modified time: 2021-09-11 22:36:55
 */
import React, { Component } from 'react';
import { createPage, base, sum } from 'nc-lightapp-front';
import getNumber from '../utils/getNumber';
import init45Template from './init/init45Template';
import search45BtnClick from './btnClicks/search45BtnClick';
import { UISTATE, TRANSFER_TYPE, URL, AREA, MAIN_ORG_FIELD, PAGECODE } from '../constance';
...
class Ref45Addline extends Component {
	constructor(props) {
		super(props);
		...
		initLang(this, [ '4004puinvoice' ], 'pu', init45Template.bind(this, this.props));
	}
	...
	render() {
		...
		let selectedShow = transferTable.getSelectedListDisplay(AREA.head45);
		...
		return (
			<div id="transferList" className="nc-bill-list">
				{!selectedShow ? (
					<div>
						<NCDiv areaCode={NCDiv.config.HEADER} className="nc-bill-header-area">
							<div className="header-title-search-area">
								<h2 className="title-search-detail">{getLangByResId(this, '4004PUINVOICE-000034')}</h2>
								{/* 国际化处理：采购入库单*/}
							</div>
					...
					/> */}
							<NCToggleViewBtn
								...
								onClick={() => {
									if (!this.props.meta.getMeta()[AREA.view45]) {
										init45Template.call(this); //加载主子拉平模板
									}
									this.props.transferTable.changeViewType(AREA.view45);
				...
				<div className="nc-bill-transferTable-area">
					{createTransferTable({
						totalKey: [ 'ninvoicenum', 'ninvoicemny' ],
						totalTitle: totalTitle,
						// dataSource: refDataSource,
						// tableType: 'simple',
						headTableId: AREA.head45, //表格组件id
						bodyTableId: AREA.body45, //子表模板id
						...
						onTransferBtnClick: (ids) => {
							...
							// 	status: UISTATE.edit,
							// 	qTempletid_50: this.state.qTempletid_45,
						...
						onChangeViewClick: () => {
							//点击切换视图钩子函数
							if (!this.props.meta.getMeta()[AREA.view45]) {
								init45Template(this.props); //加载主子拉平模板
							}
							this.props.transferTable.changeViewType(AREA.view45);
							this.setState({ toggleViewStatus: !this.state.toggleViewStatus });
						},
...
Ref45Addline = createPage({})(Ref45Addline);
export default Ref45Addline;
