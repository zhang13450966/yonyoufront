 ...
 * @Last Modified time: 2020-08-19 11:11:56
 */
...
export default function clickSerachBtn(queryInfo, value) {
	...
	if (queryInfo) {
		...
		queryInfo21 = this.props.search.getQueryInfo(AREA.searchAll, false);
		queryInfo21.userdefObj = { transType: transType };
		// setDefData(COMMON.TransferCacheKey, AREA.searchAll, queryInfo21);
		queryInfo21.querycondition = null;

		queryInfo45 = this.props.search.getQueryInfo(AREA.searchAll, false);
		queryInfo45.userdefObj = { transType: transType };
		queryInfo45.querycondition = null;
		queryInfo4T = this.props.search.getQueryInfo(AREA.searchAll, false);
		queryInfo4T.userdefObj = { transType: transType };
		queryInfo4T.querycondition = null;
	}
	...
	ajax({
		...
		success: (res) => {
			...
			if (success && data && data.length > 0) {
				...
				if (dest21datas) {
					let datas = [];
					for (let i = 0; i < dest21datas.length; i++) {
						let data = {
							children: dest21datas[i].body.bodyAll.rows,
							values: dest21datas[i].head.headAll.rows[0].values
						};
						datas[i] = data;
					}
				...
				if (dest45datas) {
					let datas = [];
					for (let i = 0; i < dest45datas.length; i++) {
						let data = {
							children: dest45datas[i].body.bodyAll.rows,
							values: dest45datas[i].head.headAll.rows[0].values
						};
						datas[i] = data;
					}
					this.props.transferTable.insertTransferTableValue(
				...
				if (dest4Tdatas) {
					let datas = [];
					for (let i = 0; i < dest4Tdatas.length; i++) {
						let data = {
							children: dest4Tdatas[i].body.bodyAll.rows,
							values: dest4Tdatas[i].head.headAll.rows[0].values
						};
						datas[i] = data;
					}
					this.props.transferTable.insertTransferTableValue(
	...
	if (billtype == BILLTYPE.po_order) {
		bills.forEach((bill) => {
			...
			bodys.forEach((body) => {
				...
				let ncaninvoicemny =
					body.values.norigtaxmny.value - body.values.nqtorigtaxprice.value * naccuminvoicenum;
