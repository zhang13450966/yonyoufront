 ...
 * @Last Modified time: 2019-12-04 09:47:57
 */
...
export default function clickCancelSendApBtn(skipCodes) {
	// 拼装json
	let dataInfo = construcateLightData(this.props, skipCodes);

	ajax({
		url: URL.cancelSendAp,
		data: dataInfo,
		success: (res) => {
			if (res.success && res.data) {
				if (res.data.isResume && res.data.isResume == true) {
					...
					showResumeModal.call(
						...
						skipCodes,
						res.data,
						clickCancelSendApBtn.bind(this),
						this.props
					);
					return;
				}
				if (res.data.head && res.data.body) {
					...
					showSuccessInfo(getLangByResId(this, '4004PUINVOICE-000008')); /* 国际化处理： 取消传应付成功！*/
				}
			}
		}
	});
