 ...
 * @Last Modified time: 2019-07-29 11:05:34
 */
...
export default function clickDelBtn(props) {
    ...
    if (rows.length <= 0) {
        toast({
            color: 'warning',
            content: getLangByResId(this, '4004PRAYBILL-000032') /* 国际化处理： 请选择需要删除的数据！*/
        });
        return;
    }
    // 弹出确认删除的框
    showWarningDialog(getLangByResId(this, '4004PRAYBILL-000057'), getLangByResId(this, '4004PRAYBILL-000014'), {
        /* 国际化处理： 确认要删除吗？*/
...
function backtotransfer(props, rows) {
    ...
    // 发送请求
    ajax({
        url: BUYINGREQ_LIST.batchDeleteURL,
        data: data,
        success: res => {
            if (res.success) {
                if (res.data) {
                    if (JSON.stringify(res.data.errorMessageMap || {}) != '{}') {
                        ...
                        rows.forEach((element, index) => {
                            ...
                            deleteCacheDataForList(props, formId, element.data.values.pk_praybill.value);
                        });
                    ...
                    showBatchOprMessage(
                        ...
                        res.data,
                        {},
                        getLangByResId(this, '4004PRAYBILL-000057')
                    ); /* 国际化处理： 提示*/
                    buttonController.setListButtonVisiable(this.props, true);
                } else {
                    showErrorInfo(getLangByResId(this, '4004PRAYBILL-000063')); /* 国际化处理： 删除失败！*/
                }
            } else {
                // toast({
                // 	color: 'warning',
