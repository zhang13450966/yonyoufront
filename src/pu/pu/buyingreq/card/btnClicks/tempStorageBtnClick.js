/*
 * @Author: guoylei
 * @PageInfo: 暂存
 * @Date: 2021-07-13 14:28:09
 * @Last Modified by: guoylei
 * @Last Modified time: 2021-10-21 14:58:21
 */
import { ACTIONS } from '../../../../../scmpub/scmpub/components/TempSave';
import { setDefData } from '../../../../../scmpub/scmpub/pub/cache';
import { BUYINGREQ_CARD } from '../../siconst';
import { showErrorInfo } from '../../../../../scmpub/scmpub/pub/tool/messageUtil';
import { getLangByResId } from '../../../../../scmpub/scmpub/pub/tool/multiLangUtil';
export default function temporaryStorage(props) {
	let config = {
		pagecode: BUYINGREQ_CARD.cardpageid,
		formId: BUYINGREQ_CARD.formId,
		tableId: BUYINGREQ_CARD.tableId,
		type: 'card',
		area: BUYINGREQ_CARD.tableId,
		param: 'pk_material'
	};
	setDefData(BUYINGREQ_CARD.tempCardCacheKey, 'tempsave', true);
	ACTIONS.SAVE.call(this, props, config, checkTempSave.bind(this));
}

//暂存保存草稿方法回调，可通过条件判断是否执行保存草稿
function checkTempSave() {
	let option = this.props.getUrlParam('option');
	let status = this.props.getUrlParam('status');
	if (status == 'edit' && option != 'copy') {
		showErrorInfo('', getLangByResId(this, '4004PRAYBILL-000068')); /* 国际化处理： 已保存单据不支持暂存！*/
		setDefData(BUYINGREQ_CARD.tempCardCacheKey, 'tempsave', false);
		return false;
	}
	let srcids = this.props.cardTable.getColValue(BUYINGREQ_CARD.tableId, 'csourceid');
	if (!srcids || srcids.length == 0) {
		return true;
	}
	let ischeck = false;
	srcids.forEach((srcid) => {
		if (srcid.value) {
			ischeck = true;
		}
	});
	if (ischeck) {
		showErrorInfo('', getLangByResId(this, '4004PRAYBILL-000069')); /* 国际化处理： 单据存在来源，不能支持暂存！*/
		setDefData(BUYINGREQ_CARD.tempCardCacheKey, 'tempsave', false);
		return false;
	}
	return true;
}
