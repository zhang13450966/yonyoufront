 ...
 * @Last Modified time: 2019-01-17 15:26:10
 */
...
export default function openRowBtnClick(props, text, record, index) {
	...
	if (record != undefined) {
		...
		// 发送请求
		ajax({
			url: BUYINGREQ_CARD.lineOpenURL,
			data: data,
			success: (res) => {
				if (res.success) {
					...
					showSuccessInfo(getLangByResId(this, '4004PRAYBILL-000055')); /* 国际化处理： 行打开成功！*/
				} else {
					showErrorInfo(getLangByResId(this, '4004PRAYBILL-000020')); /* 国际化处理： 行打开失败！*/
				}
			}
		});
	} else {
