 ...
 * @Last Modified time: 2021-05-21 17:11:12
 */
...
export default function print_BtnClick(props) {
	let pks = [];
	let selrows = props.table.getCheckedRows(PAGECODE.list_head);
	if (selrows.length == 0) {
		toast({
			color: 'danger',
			content: getLangByResId(this, '4004ORDERREVISE-000022') /* 国际化处理： 请选择要打印的订单！*/
		});
		return;
	}
	selrows.forEach((row) => {
