 ...
 * @Last Modified time: 2021-08-26 18:10:33
 */
...
import { updateCacheDataForList, updateCacheDataForListWhenChangePK } from '../../../../../scmpub/scmpub/pub/cache';
...
export default function commit(props, record, index, assign) {
	...
	if (record && record.pk_order) {
		...
		let rows = props.table.getCheckedRows(PAGECODE.list_head);
		// 如果没有选中行，则提示并返回，不进行任何操作
		if (rows.length <= 0) {
			toast({
				color: 'warning',
				content: getLangByResId(this, '4004ORDERREVISE-000034') /* 国际化处理： 请选择需要提交的数据！*/
			});
			return;
		}
		rows.map((item) => {
	...
	ajax({
		...
		success: (res) => {
			...
			if (res.success) {
				...
				if (
					res.data.sucessVOs &&
					res.data.sucessVOs.list_head.rows[0].values.forderstatus.value == FIELD.approved
				) {
					...
					updateCacheDataForList(props, PAGECODE.list_head, FIELD.pk_order, res.data, indexs);
				}

				buttonController.initButtons.call(this, props);
				/* 国际化处理： 提交成功！*/
				if (record && record.pk_order) {
					showSuccessInfo(getLangByResId(this, '4004ORDERREVISE-000035'));
				} else {
					//批量
					showBatchOprMessage(
						getLangByResId(this, '4004ORDERREVISE-000036'),
						res.data,
						{},
						getLangByResId(this, '4004ORDERREVISE-000036')
					);
