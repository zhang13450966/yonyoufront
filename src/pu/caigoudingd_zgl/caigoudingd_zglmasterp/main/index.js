...
export class ApplicationPage extends Component{
	...
	*/
    ...
    initMeta = (meta) => {
        ...
        meta['caigoudingd_zglSlave0Sub'].items.push({
            ...
            render: (text, record, index) => { // 渲染方法
                ...
                if (editmode != EDITMODE_BROWSE) {
                    oprButtons.push('caigoudingd_zglSlave0_Opr_DelLine');
                    oprButtons.push('caigoudingd_zglSlave0_Opr_CopyLine');
                }
                oprButtons.push('caigoudingd_zglSlave0_Opr_ExpandLine');
                if (editmode != EDITMODE_BROWSE && copiedRows['caigoudingd_zglSlave0Sub']) {
                    oprButtons = ['caigoudingd_zglSlave0_Opr_PastNextLine'];
                }
                // 创建操作列API
                return this.props.button.createOprationButton(oprButtons, {
                    area: 'caigoudingd_zglSlave0_Opr', // 按钮区域编码
        ...
        // 交易类型参照过滤
        meta['caigoudingd_zglMasterForm'].items.find(item => item.attrcode == FIELDS.TRANSTYPEPK).queryCondition = () => ({
    ...
    createState = () => {
        let state = {
            ...
            card: { // 卡片页面
                ...
                tabs: {
                    activeKey: 'caigoudingd_zglSlave0Sub',
                    onTabClick: (key) => {
                ...
                formlist: { // 子表表格
                    'caigoudingd_zglSlave0Sub'/*区域编码*/: {
                        ...
                        button: { // 肩部按钮
                            area: 'caigoudingd_zglSlave0', // 区域编码
    ...
    */
    onCardTableCopy = (tableId, row) => {
        ...
        this.state.card.copiedRows[tableId] = copiedRows;
        this.setState(this.state, () => {
    ...
    */
    onCardTablePast = (tableId, row) => {
        ...
        copiedRows[tableId].forEach(row => {
	...
	*/
    onCardTableCopyCancel = (tableId) => {
        delete this.state.card.copiedRows[tableId];
        this.setState(this.state, () => {
            this.updateBtnStatus();
        })
    }
...
const config = {
    title: '采购订单_张广禄审批',
    pagecode: 'TRH10202zglP_caigoudingd_zglMasterP',
    appcode: 'TRH10202zglP',
    domainName: 'pu',
    version: 2207
};
