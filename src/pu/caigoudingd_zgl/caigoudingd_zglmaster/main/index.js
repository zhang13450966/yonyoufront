...
const URLS = {  // 请求路径
    printUrl: '/nccloud/pu/caigoudingd_zgl/PrintCaigoudingd_zglMasterVOAction.do',
	auditUrl: '/nccloud/pu/caigoudingd_zgl/PFlowCaigoudingd_zglMasterVOAction.do',//审批流提交收回等请求路径
	querypuzglUrl: '/nccloud/pu/caigoudingd_zgl/QuerypuzglForTRH102ZGLCaigoudingd_zglMasterVOAction.do',//拉单查询pu请求路径
	transferUrl: '/nccloud/pu/caigoudingd_zgl/TransferCaigoudingd_zglMasterVOAction.do',//转单请求路径
    saveUrl: '/nccloud/pu/caigoudingd_zgl/SaveCaigoudingd_zglMasterVOAction.do',
    deleteUrl: '/nccloud/pu/caigoudingd_zgl/DeleteCaigoudingd_zglMasterVOAction.do',
    listUrl: '/nccloud/pu/caigoudingd_zgl/ListCaigoudingd_zglMasterVOAction.do',
    loadUrl: '/nccloud/pu/caigoudingd_zgl/LoadCaigoudingd_zglMasterVOAction.do',
    addUrl: '/nccloud/pu/caigoudingd_zgl/AddCaigoudingd_zglMasterVOAction.do',
    editUrl: '/nccloud/pu/caigoudingd_zgl/EditCaigoudingd_zglMasterVOAction.do',
    copyUrl: '/nccloud/pu/caigoudingd_zgl/CopyCaigoudingd_zglMasterVOAction.do'
};
...
const ACTIONS = {  // 按钮编码
    ...
    FETCHSAVE: 'FetchSave',  //拉单保存
    FETCHREFRESH: 'FetchRefresh',  //拉单刷新
    FETCHEDIT: 'FetchEdit',//拉单修改
    FETCHCANCEL: 'FetchCancel',  //拉单取消
    FETCHBTN: 'FetchBtn',  //拉单
    FETCHEXIT: 'FetchExit', //退出转单
...
const FIELDS = {  // 字段编码
    ...
    CODE: 'code',
    PK_ORG: 'pk_org',
    PRIMARYKEY: 'pk_cgdingdan_zglmaster'
}
...
const SHOWMODE_FETCHLIST = 'fetchlist';  //拉单查询数据，显示上游数据列表界面
const SHOWMODE_FETCHCARD = 'fetchcard';  //拉单生成下游单据卡片界面

const EXCELBILLTYPE = 'CAIGOUDINGD_ZGLMASTER_TRH10202zgl';  // 导入导出单据类型

const PRINTNODEKEY_LIST = 'TRH10202zgl01';  // 列表打印模板标识
const PRINTNODEKEY_CARD = 'TRH10202zgl02';  // 卡片打印模板标识

const BILLTYPE = 'TRH102ZGL';
...
export class ApplicationPage extends Component{
    ...
    constructor(props) {
         ...
         */
        this.props.use.search('caigoudingd_zglMasterQuery', 'pu_caigouhet_zglMaster_transquery');
        this.props.use.form('caigoudingd_zglMasterForm');
        this.props.use.table('caigoudingd_zglMasterList', 'puzgl');
        this.props.use.cardTable('caigoudingd_zglSlave0Sub');
    ...
    pageReady = () => {
        ...
        this.props.search.setSearchValByField('caigoudingd_zglMasterQuery', FIELDS.PK_ORG, {display: context.org_Name, value: context.pk_org});
        // 交易类型发布小应用
        let {context: {paramMap} = {}} = this.state;
        if (paramMap && paramMap.pk_transtype) {
            let meta = this.props.meta.getMeta();
            meta['caigoudingd_zglMasterQuery'].items.find(item => {
                if (item.attrcode == FIELDS.TRANSTYPEPK) {
                    item.disabled = true;
                    return true;
                }
            });
            meta['caigoudingd_zglMasterForm'].items.find(item => {
                if (item.attrcode == FIELDS.TRANSTYPEPK) {
                    item.disabled = true;
                    return true;
                }
            });
            this.props.search.setSearchValByField('caigoudingd_zglMasterQuery', FIELDS.TRANSTYPEPK, {display: paramMap.transtype_name, value: paramMap.pk_transtype});
    ...
    initMeta = (meta) => {
        // 添加列表行操作列
        meta['caigoudingd_zglMasterList'].items.push({
            attrcode: 'opr', // 列标识, 固定
        ...
        // 添加卡片行操作列
        meta['caigoudingd_zglSlave0Sub'].items.push({
            attrcode: 'opr', // 列标识, 固定
            itemtype:'customer', // 列类型, 固定
            label: '操作', // 列名
            width: 200, // 列宽度
            ...
            render: (text, record, index) => { // 渲染方法
                ...
                let oprButtons = [];
                if (copiedArea) {
                    oprButtons = copiedArea == 'caigoudingd_zglSlave0Sub' ? ['caigoudingd_zglSlave0Sub_PastNextLineOpr'] : [];
                } else {
                    if (editmode != EDITMODE_BROWSE) {
                        oprButtons.push('caigoudingd_zglSlave0Sub_DelLineOpr');
                        oprButtons.push('caigoudingd_zglSlave0Sub_CopyLineOpr');
                    }
                    oprButtons.push('caigoudingd_zglSlave0Sub_ExpandLineOpr');
                }
                // 创建操作列API
                return this.props.button.createOprationButton(oprButtons, {
                    area: 'caigoudingd_zglSlave0SubOpr', // 按钮区域编码
                    buttonLimit: 3, // 允许的按钮最多显示数量
                    onButtonClick: (props, btncode) => this.onBtnClickFormlist(props, btncode, 'caigoudingd_zglSlave0Sub', {data: record, index}) // 点击按钮
                });
            }
        });
        ...
        meta['caigoudingd_zglMasterForm'].items.find(item => {
            if (item.attrcode == FIELDS.BUSITYPE) {
                item.queryCondition = () => {
                    let transtype = this.props.form.getFormItemsValue('caigoudingd_zglMasterForm', FIELDS.TRANSTYPE).value;
                    return {
                        billtype: BILLTYPE,
                        transtype,
        ...
        // 云原生适配，saga开头的属性全部隐藏
        meta['caigoudingd_zglMasterForm'].items.forEach(item => {
            if (item.attrcode.startsWith('saga')) {
                item.visible = false;
                item.disabled = true;
            }
        });
        meta['caigoudingd_zglMasterList'].items.forEach(item => {
            if (item.attrcode.startsWith('saga')) {
                item.visible = false;
                item.disabled = true;
            }
    ...
    createState = () => {
        let state = {
            ...
            list: { // 列表页面
                search: { // 查询区
                    area: 'caigoudingd_zglMasterQuery', // 区域编码
                    clickSearchBtn: this.onSearch // 点击查询按钮
                },
                table: { // 主表表格
                    allpks: [], // 列表查询结果行pk数组
                    area: 'caigoudingd_zglMasterList', // 区域编码
                    showCheck: true, // 是否显示多选框
                    showIndex: true, // 是否显示行序号
                    adaptionHeight: true, // 是否自适应浏览器高度
            ...
            card: { // 卡片页面
                form: { // 表单
                    area: 'caigoudingd_zglMasterForm', // 区域编码
                    onBeforeEvent: this.onBeforeEditForm, // 编辑前事件
                    onAfterEvent: this.onAfterEditForm, // 编辑后事件
                },
                formlist: { // 子表表格
                    'caigoudingd_zglSlave0Sub'/*区域编码*/: {
                        area: 'caigoudingd_zglSlave0Sub', // 区域编码
                        showIndex: true, // 是否显示行序号
                        showCheck: true, // 是否显示多选框
                        adaptionHeight: true, // 是否自适应浏览器高度
                        onAfterEvent: this.onAfterEditFormlist, // 编辑后事件
                        ...
                        onSelectedAll: this.onSelectedChangeFormlist, // 点击全选框
                        selectedChange: this.onSelectedChangeFormlist, // 选择框的选中行改变
                        modelSave: () => this.onFormlistModelSave('caigoudingd_zglSlave0Sub'), // 整单保存
                        tableHead: () => this.state.card.formlist['caigoudingd_zglSlave0Sub'].button.render(), // 肩部渲染，主要用于渲染肩部操作按钮，调用肩部按钮的渲染方法
                        button: { // 肩部按钮
                            area: 'caigoudingd_zglSlave0Sub', // 区域编码
                            onButtonClick: (props, btnCode) => this.onBtnClickFormlist(props, btnCode, 'caigoudingd_zglSlave0Sub'), // 点击按钮
                            render: () => { // 渲染方法
                                return (
                                    <div className="shoulder-definition-area">
                                        <div className="btn-group">
                                            {/*按钮*/}
                                            {this.props.button.createButtonApp(this.state.card.formlist['caigoudingd_zglSlave0Sub'].button)}
                                        </div>
                                    </div>
                                );
                            }
        ...
        Utils.apply(state, {
            fetchlist: { // 拉单列表
                ...
                fetchHead: { // 头部标题
                    title: '选择pu' ,  //头部标题
                    backBtnClick: this.onBackFetchlist, // 点击返回
                ...
                ncTabs: {  //NCTabs配置
                    defaultActiveKey: "puzgl", // 默认选中页签标识
                    activeKey: "puzgl", // 当前选中页签标识
                    onChange: (key) => { // 选中页签发生改变
                        this.state.fetchlist.ncTabs.activeKey = key;
                        this.setState(this.state);
                    },  //tabPanel改变的时候的回调函数
                    onTabClick: EMPTY_FN,  //tab 被点击的回调
                    tabPanes: [ // 页签详情
                        {
                            headIdName: 'pk_caigouhet_zglmaster',//主表主键名称
                            bodyIdName: '',//子表主键名称
                            nctabpane: {//页签配置
                                key: 'puzgl',//页签的key
                                tab: '主实体_拉单上游列表区域' //页签名字
                            },
                            search: { // 查询区
                                area: 'pu_caigouhet_zglMaster_transquery', // 区域编码
                                clickSearchBtn: () => { // 点击查询
                                    this.transferSearch({}, (data,num) => {//查询后的回调
                                        if (num && num > 0) {
                                ...
                                renderCompleteEvent: () => { // 查询区渲染完后回调事件
									let { context } = this.state;
									if (context && context.pk_org) { // 查询区设置当前用户业务设置中的默认业务单元
                                        let searchCondition = [
                                            { field: 'pk_org', oprtype: '=', value: { firstvalue: context.pk_org }, display: context.org_Name }
                                        ];
                                        this.props.search.setSearchValue('pu_caigouhet_zglMaster_transquery', searchCondition);
                                    }
                                },
                                */
                                onAfterEvent: (key,value) =>{ // 编辑前事件
                                    //拉单的查询区编辑后事件处理。
                                }
                            },
                            transferTable: { // 转单表格
                                dataSource: 'pu.pu.pu_caigouhet_zglMaster_transfercache',  //缓存数据的key
                                headTableId: 'pu_caigouhet_zgl_pu_caigouhet_zglMaster_list', //表格组件id
                                bodyTableId: undefined, //子表模板id
                                fullTableId: undefined, //主子拉平的id
                                tableType: 'simple',//表格显示类型 nest:主子表 full:主子拉平 simple:单表
								showSeletedBtn:true,//左下角“查看已选”是否显示
                                showChangeViewBtn:false,//显示主子拉平的按钮是否显示
                                transferBtnText: '转单', //转单按钮显示文字
                                containerSelector: '#transferList',//父组件容器
                                billType: 'puzgl', // 单据类型
                                onTransferBtnClick: (ids) => { // 点击转单按钮
                                    let area = this.getCardAreaCode();
                                    let params = {
                                        data: Utils.onTransferWrapper({props: this.props, fetchList: this.state.fetchlist}),
                                        pageCode: this.config.pagecode,
                                        formId: area.formId,
                                        formListIds: area.tableId
                                    };
                                    this.onTransfer(params);
                                },
                                onChangeViewClick: () => { // 拉单点击切换视图
                                    this.props.transferTable.changeViewType();
                                    this.state.fetchlist.toogleViewBtn.expand = !this.state.fetchlist.toogleViewBtn.expand;
                                    this.setState(this.state);
                                },
                                componentInitFinished: () => { // 缓存数据赋值成功
            ...
            fetchcard: {//拉单卡片界面
                fetchHead: {//拉单卡片头部配置（）
                    title: '采购订单_zgl' ,// 标题
                    backBtnClick: this.onBackFetchcard // 点击返回
                },
                fetchButton: { // 按钮区域
                ...
                tranlist: { // 转单列表
                    headcode: 'caigoudingd_zglMasterForm',//表头的区域编码
                    bodycode: [  'caigoudingd_zglSlave0Sub',  ],//表体区域编码
                    transferListId: 'caigoudingd_zglMaster_translist',//转单列表 id
    ...
    render() {
        ...
        const renderCard = () => {
            ...
            let {'caigoudingd_zglSlave0Sub' : tableConfig, ...other} = newFormlist;
            return (
		...
		let renderFetchList = () => {
            ...
            let {fetchHead, fetchButton, toggleViewBtn,ncTabs} = fetchlist;
            let {tabPanes} = ncTabs;
            if (showmode != SHOWMODE_FETCHLIST) {
                return '';
            }
			let transferTableCfgs = tabPanes.filter(item => item.nctabpane.key == ncTabs.activeKey).map(item=>item.transferTable);
            let selectedShow = this.props.transferTable.getSelectedListDisplay(transferTableCfgs[0].headTableId);
            if(selectedShow){//查看已选时界面
                return (
                <div id="transferList" className="nc-bill-list">
                    <div className="nc-bill-transferTable-area">
                        {this.props.transferTable.createTransferTable(transferTableCfgs[0])}
                    </div>
                </div>
                );
            }
            return (
                <div className="nc-bill-list">
                    ...
                    <NCDiv className="nc-bill-header-area" areaCode={NCDiv.config.HEADER}>
                        {/* 标题 title */} 
						<div className="header-title-search-area">{this.props.BillHeadInfo.createBillHeadInfo(fetchHead)}</div>
                        {/* 按钮区  btn-group */}
						<div className="header-button-area">
                            {this.props.button.createButtonApp(fetchButton)}
                            {/* 主子拉平的按钮 */}
                            {/*<div> <NCToggleViewBtn {...toggleViewBtn} /></div>*/}
						</div>
                    </NCDiv>
					...
					<NCTabs  {...ncTabs} >
						{
							tabPanes.map(item => {
								return (
									<NCTabs.NCTabPane {...item.nctabpane}>
										<NCDiv areaCode={NCDiv.config.SEARCH} className="nc-bill-search-area">
											{ this.props.search.NCCreateSearch(item.search.area,item.search  ) }
										</NCDiv>
										<NCDiv className="nc-bill-table-area">
											{ this.props.transferTable.createTransferTable(item.transferTable) }
										</NCDiv>
									</NCTabs.NCTabPane>
								);//return (
							})//tabPanes.map(..
    ...
    updateBtnStatus = ({callback = EMPTY_FN} = {}) => {
		...
		// 拉单按钮控制
		Object.assign(btnVisible, {
            [ACTIONS.FETCHSAVE]: !isBrowse && showmode == SHOWMODE_FETCHCARD,
            [ACTIONS.FETCHCANCEL]: !isBrowse && showmode == SHOWMODE_FETCHCARD,
            [ACTIONS.FETCHEDIT]: isBrowse && showmode == SHOWMODE_FETCHCARD,
            [ACTIONS.FETCHBTN] : isBrowse && showmode != SHOWMODE_FETCHCARD,
            [ACTIONS.FETCHEXIT]: showmode == SHOWMODE_FETCHCARD

        });
        Object.assign(btnDisabled, {
            [ACTIONS.FETCHSAVE]: hasCopiedRows,
            [ACTIONS.FETCHCANCEL]: hasCopiedRows,
            [ACTIONS.FETCHEDIT]: false,
            [ACTIONS.FETCHBTN] : false,
            [ACTIONS.FETCHEXIT]: hasCopiedRows
		});
        Object.assign(btnVisible, {
    ...
    onBtnClickHead = (props, btnCode, param = {}) => {
        switch (btnCode) {
			...
			case ACTIONS.UNCOMMIT: //点击收回
                ...
                break;
			case ACTIONS.FETCHBTN: // 点击拉单
				this.onFetchBtnClick();
				break;
			case ACTIONS.FETCHREFRESH: // 点击拉单页面的刷新
				this.onFetchRefresh();
				break;
			case ACTIONS.FETCHEDIT: // 点击拉单页面的编辑
            	this.onFetchEdit();
                break;
			case ACTIONS.FETCHSAVE: // 点击拉单页面的保存
				this.onFetchSave();
                break;
            case ACTIONS.FETCHCANCEL: // 点击拉单页面的取消
            	this.onFetchCancel();
                break;
            case ACTIONS.FETCHEXIT: // 点击退出转单
            	this.onFetchExit();
                break;
            ...
            this.save({
                ...
                callback: ({data}) => {
                    ...
                    this.setState(this.state, () => {
                        ...
                        // 拉单卡片整单保存需要更新转单组件状态
                        if (this.state.showmode == SHOWMODE_FETCHCARD) {
                            let {tranlist: {transferListId}, selectIndex} = this.state.fetchcard;
                            // 更新转单组件的数据
                            this.props.transferTable.setTransferListValueByIndex(transferListId, data, selectIndex);
                            // 更新当前转单数据的状态为已处理
                            this.props.transferTable.updateTransformFormStatus(transferListId, {status: true, index: selectIndex});
                        }
                        // 提示
                        toast({color: 'success', content: '保存成功'});
                    });
    ...
    beforePastCardTable = (tableId, row) => {
        switch (tableId) {
            case 'caigoudingd_zglSlave0Sub':
                row.values['pk_cgdingdan_zglslave0'] = {value: '', display: ''};
    ...
    onFetchBtnClick = () => {
        // 设置页面模式为拉单列表浏览态
        this.state.showmode = SHOWMODE_FETCHLIST;
        this.state.editmode = EDITMODE_BROWSE;
        this.setState(this.state, () => {
            let {fetchlist} = this.state;
            let {fetchHead, fetchButton, ncTabs} = fetchlist;
            let {tabPanes, ...others} = ncTabs;
            tabPanes.forEach(tabPane => {
                let {transferTable, headIdName, bodyIdName} = tabPane;
                let {headTableId, bodyTableId} = transferTable;
                this.props.transferTable.setTransferTableValue(headTableId, bodyTableId, [], headIdName, bodyIdName);
            });
     ...
     *      拉单卡片界面返回按钮处理
     */
	onBackFetchcard = () => {//拉单卡片返回按钮点击处理
        let {showmode, editmode, fetchcard} = this.state;
        let {tranlist: {transferListId}} = fetchcard;
        // 拉单卡片是否已处理全部数据
        let isAllhandler = this.props.transferTable.getTransformFormStatus(transferListId);
        if (!isAllhandler) {
            let handleBackToFetchlist = () => {
                // 设置页面模式为拉单列表浏览态
                this.state.showmode = SHOWMODE_FETCHLIST;
                this.state.editmode = EDITMODE_BROWSE;
                this.setState(this.state, () => {
            ...
            promptBox({
                color: 'warning',
                title: '确认返回',
                content: '有未保存的单据，确定要返回吗?',
                beSureBtnClick: handleBackToFetchlist
            });
            return;
        }
        // 设置页面模式为列表浏览态
     ...
     * @param callback
     */
	transferSearch = (param = {}, callback = EMPTY_FN) => {
		let {showmode, editmode, fetchlist} = this.state;
		let {ncTabs: {activeKey, tabPanes}} = fetchlist;

		// 构建拉单列表当前上游单据信息
        let otherParams = tabPanes.filter(item => item.nctabpane.key == activeKey).map(item => {
            return {
                searchid: item.search && item.search.area || undefined,//查询区域id
                tableType: item.transferTable && item.transferTable.tableType || undefined,
                formId: item.transferTable && item.transferTable.headTableId || undefined, // 主表id
                tableId: [item.transferTable && item.transferTable.bodyTableId] || [], // 子表id，后端子表为数组
                userJson: {code: activeKey}, // 页签编码
                headIdName: item.headIdName, // 主表主键名称
                bodyIdName: item.bodyIdName // 子表主键名称
            }
        });
        if(!otherParams || !otherParams.length) {
            console.error("页签错误！未找到页签：" + activeKey);
            return;
        }
        otherParams = otherParams[0];
		
		// 获取查询条件
        let queryInfo = this.props.search.getQueryInfo(otherParams.searchid);
        if (!queryInfo) {
            return;
        }
        // 构建查询参数
        ...
        // 查询请求
		ajax({
            url: URLS['query' + activeKey + 'Url'],
            data: paramData,
            loading: true,
            success: (res) => {
                let data = res.data && res.data.data;
                let {formId,tableType, tableId, headIdName, bodyIdName} = otherParams;
                // 获取上游单据子表区域编码，目前仅支持拉一个子表
                tableId = tableId && tableId[0] ? tableId[0] : null;
                let num = 0;
                if (tableType == 'simple') {
                    let fetchListData = data && data[formId] || { rows: [] };
                    num = fetchListData.rows.length;
                    this.props.transferTable.setTransferTableValue(formId, tableId, fetchListData, headIdName, bodyIdName);
                } else {
                    ...
                    this.props.transferTable.setTransferTableValue(formId, tableId, data || [], headIdName, bodyIdName);
                }
                callback(data, num);
            }
        });
     ...
     * @param callback
     */
    onTransfer = (param = {}, callback = EMPTY_FN) => {
    	let {tranlist} = this.state.fetchcard;
        if (!param.data || param.data.length == 0) { // 未选中上游数据
            toast({color: 'warning', content: '请选择数据！'});
            return;
        }
        // 转单请求
        ajax({
            url: URLS.transferUrl,
            data: param,
            loading: true, // 是否显示加载中动画
            success: (res) => {
                let {data: {data, externalData: {billCodeContext} = {}} = {}} = res;
                if (!data) {
                    return;
                ...
                // 设置页面模式为拉单卡片编辑态
                this.state.showmode = SHOWMODE_FETCHCARD;
                this.state.editmode = EDITMODE_EDIT;
                this.setState(this.state, () => {
                    // 设置转单组件数据
                    this.props.transferTable.setTransferListValue(tranlist.transferListId, Utils.transferDataWapper(data));
                    // 处理编码规则
                    this.handleCodeRule({billCodeContext});
                    // 更新按钮状态
                    this.updateBtnStatus();
     ...
     */
    onFetchEdit = () => {
    	let {transferListId} = this.state.fetchcard.tranlist;
    	let {fetchcard: {selectIndex}} = this.state;
    	// 设置页面模式为拉单卡片编辑态
    	this.state.showmode = SHOWMODE_FETCHCARD;
    	this.state.editmode = EDITMODE_EDIT;
    	this.setState(this.state, () => {
            ...
            this.updateCardStatus({areaInfo, editmode: EDITMODE_EDIT});
            // 更新当前转单数据的状态为未处理
            this.props.transferTable.updateTransformFormStatus(transferListId, {status: false, index: selectIndex});
            // 更新按钮状态
            this.updateBtnStatus();
        });
    }
    ...
    onFetchCancel = () => {
    	let {transferListId} = this.state.fetchcard.tranlist;
    	// 获取当前转单数据条数
    	let count = this.props.transferTable.getTransformFormAmount(transferListId);
        if (count == 1) { // 只有一条转单数据
            // 设置页面模式为列表浏览态
            this.state.showmode = SHOWMODE_LIST;
            this.state.editmode = EDITMODE_BROWSE;
            ...
            // 从当前转单列表中删除
            this.props.transferTable.setTransformFormStatus(transferListId, {status: false});
        }
    }
    /**
     * 方法功能：
     ...
     * @param index
     */
    onTransferItemSelected = (record, status, index) => {//转单后卡片左侧缩略图选中点击处理
		let {fetchcard} = this.state;
		let {selectIndex, tranlist} = fetchcard;
        ...
        if(index != selectIndex && selectIndex < transferCount){
            //点击左侧缩略图前编辑的卡片数据
            let lastRecordInfo = this.getCardData();
            //根据序号更新缩略图数据
            this.props.transferTable.setTransferListValueByIndex(transferListId, Utils.transferDataWapper(lastRecordInfo), selectIndex);
        }
        fetchcard.selectIndex = index;
        // 根据主键标记当前转单数据是否未保存
        let isAdd = true;
        let pk = record && record.head && record.head[form.area].rows[0].values[FIELDS.PRIMARYKEY];
        pk && (isAdd = pk && pk.value ? false : true);
        // 设置页面模式为拉单卡片，编辑模式根据是新增还是修改进行设置
        this.state.showmode = SHOWMODE_FETCHCARD;
        let editmode = status ? EDITMODE_BROWSE : (isAdd ? EDITMODE_ADD : EDITMODE_EDIT);
        this.state.editmode = editmode;
        this.setState(this.state, () => {
            // 获取卡片区域编码
            let areaInfo = this.getCardAreaCode();
            // 设置编辑状态
            this.updateCardStatus({areaInfo, editmode});
            // 填充卡片数据或清空卡片数据
            record ? this.fillCardData({data: record}) : this.clearCardData();
            // 更新按钮状态
            this.updateBtnStatus();
        });
	}
...
ApplicationPage = createPage({
    ...
    billinfo: {
        billtype: 'extcard',
        pagecode: 'TRH10202zgl_caigoudingd_zglMaster',
        headcode: 'caigoudingd_zglMasterForm',
        bodycode: ['caigoudingd_zglSlave0Sub']
    },
    orderOfHotKey: ['caigoudingd_zglMasterForm', 'caigoudingd_zglSlave0Sub']
})(ApplicationPage);
