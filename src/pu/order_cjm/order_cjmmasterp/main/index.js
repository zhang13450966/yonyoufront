...
export class ApplicationPage extends Component{
	...
	*/
    ...
    initMeta = (meta) => {
        ...
        meta['order_cjmSlave0Sub'].items.push({
            ...
            render: (text, record, index) => { // 渲染方法
                ...
                if (editmode != EDITMODE_BROWSE) {
                    oprButtons.push('order_cjmSlave0_Opr_DelLine');
                    oprButtons.push('order_cjmSlave0_Opr_CopyLine');
                }
                oprButtons.push('order_cjmSlave0_Opr_ExpandLine');
                if (editmode != EDITMODE_BROWSE && copiedRows['order_cjmSlave0Sub']) {
                    oprButtons = ['order_cjmSlave0_Opr_PastNextLine'];
                }
                // 创建操作列API
                return this.props.button.createOprationButton(oprButtons, {
                    area: 'order_cjmSlave0_Opr', // 按钮区域编码
        ...
        // 交易类型参照过滤
        meta['order_cjmMasterForm'].items.find(item => item.attrcode == FIELDS.TRANSTYPEPK).queryCondition = () => ({
...
const config = {
    title: '采购订单-陈金明审批',
    pagecode: '4004900H1P_order_cjmMasterP',
    appcode: '4004900H1P',
    domainName: 'pu',
    version: 2207
};
