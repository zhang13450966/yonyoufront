...
const URLS = {  // 请求路径
    printUrl: '/nccloud/pu/order_cjm/PrintOrder_cjmMasterVOAction.do',
    listSlaveUrl: '/nccloud/pu/order_cjm/ListSlaveOrder_cjmMasterVOAction.do',
	auditUrl: '/nccloud/pu/order_cjm/PFlowOrder_cjmMasterVOAction.do',//审批流提交收回等请求路径
    saveUrl: '/nccloud/pu/order_cjm/SaveOrder_cjmMasterVOAction.do',
    deleteUrl: '/nccloud/pu/order_cjm/DeleteOrder_cjmMasterVOAction.do',
    listUrl: '/nccloud/pu/order_cjm/ListOrder_cjmMasterVOAction.do',
    loadUrl: '/nccloud/pu/order_cjm/LoadOrder_cjmMasterVOAction.do',
    addUrl: '/nccloud/pu/order_cjm/AddOrder_cjmMasterVOAction.do',
    editUrl: '/nccloud/pu/order_cjm/EditOrder_cjmMasterVOAction.do',
    copyUrl: '/nccloud/pu/order_cjm/CopyOrder_cjmMasterVOAction.do'
};
...
const EXCELBILLTYPE = 'ORDER_CJMMASTER_4004900H1';  // 导入导出单据类型

const PRINTNODEKEY_LIST = '4004900H101';  // 列表打印模板标识
const PRINTNODEKEY_CARD = '4004900H102';  // 卡片打印模板标识
...
export class ApplicationPage extends Component{
    ...
    constructor(props) {
         ...
         */
        this.props.use.search('order_cjmMasterQuery');
        this.props.use.form('order_cjmMasterForm');
        this.props.use.table('order_cjmMasterList', 'order_cjmSlave0List');
        this.props.use.cardTable('order_cjmSlave0Sub');
    ...
    pageReady = () => {
        ...
        this.props.search.setSearchValByField('order_cjmMasterQuery', FIELDS.PK_ORG, {display: context.org_Name, value: context.pk_org});
        // 交易类型发布小应用
        let {context: {paramMap} = {}} = this.state;
        if (paramMap && paramMap.pk_transtype) {
            let meta = this.props.meta.getMeta();
            meta['order_cjmMasterQuery'].items.find(item => {
                if (item.attrcode == FIELDS.TRANSTYPEPK) {
                    item.disabled = true;
                    return true;
                }
            });
            meta['order_cjmMasterForm'].items.find(item => {
                if (item.attrcode == FIELDS.TRANSTYPEPK) {
                    item.disabled = true;
                    return true;
                }
            });
            this.props.search.setSearchValByField('order_cjmMasterQuery', FIELDS.TRANSTYPEPK, {display: paramMap.transtype_name, value: paramMap.pk_transtype});
    ...
    initMeta = (meta) => {
        // 添加列表行操作列
        meta['order_cjmMasterList'].items.push({
            attrcode: 'opr', // 列标识, 固定
        ...
        // 添加卡片行操作列
        meta['order_cjmSlave0Sub'].items.push({
            attrcode: 'opr', // 列标识, 固定
            itemtype:'customer', // 列类型, 固定
            label: '操作', // 列名
            width: 200, // 列宽度
            ...
            render: (text, record, index) => { // 渲染方法
                ...
                let oprButtons = [];
                if (copiedArea) {
                    oprButtons = copiedArea == 'order_cjmSlave0Sub' ? ['order_cjmSlave0Sub_PastNextLineOpr'] : [];
                } else {
                    if (editmode != EDITMODE_BROWSE) {
                        oprButtons.push('order_cjmSlave0Sub_DelLineOpr');
                        oprButtons.push('order_cjmSlave0Sub_CopyLineOpr');
                    }
                    oprButtons.push('order_cjmSlave0Sub_ExpandLineOpr');
                }
                // 创建操作列API
                return this.props.button.createOprationButton(oprButtons, {
                    area: 'order_cjmSlave0SubOpr', // 按钮区域编码
                    buttonLimit: 3, // 允许的按钮最多显示数量
                    onButtonClick: (props, btncode) => this.onBtnClickFormlist(props, btncode, 'order_cjmSlave0Sub', {data: record, index}) // 点击按钮
                });
            }
        });
        ...
        meta['order_cjmMasterForm'].items.find(item => {
            if (item.attrcode == FIELDS.BUSITYPE) {
                item.queryCondition = () => {
                    let transtype = this.props.form.getFormItemsValue('order_cjmMasterForm', FIELDS.TRANSTYPE).value;
                    return {
                        billtype: BILLTYPE,
                        transtype,
        ...
        // 云原生适配，saga开头的属性全部隐藏
        meta['order_cjmMasterForm'].items.forEach(item => {
            if (item.attrcode.startsWith('saga')) {
                item.visible = false;
                item.disabled = true;
            }
        });
        meta['order_cjmMasterList'].items.forEach(item => {
            if (item.attrcode.startsWith('saga')) {
                item.visible = false;
                item.disabled = true;
            }
    ...
    createState = () => {
        let state = {
            ...
            list: { // 列表页面
                search: { // 查询区
                    area: 'order_cjmMasterQuery', // 区域编码
                    clickSearchBtn: this.onSearch // 点击查询按钮
                },
                table: { // 主表表格
                    allpks: [], // 列表查询结果行pk数组
                    area: 'order_cjmMasterList', // 区域编码
                    showCheck: true, // 是否显示多选框
                    showIndex: true, // 是否显示行序号
                    adaptionHeight: true, // 是否自适应浏览器高度
                ...
                tablelist: { // 列表子表详情
                    tablelistRow: {}, // 列表子表明细对应主表行数据，只有当点击主表表格行时才会给这个属性设值，重新查询列表会清空 格式：{values: {}}
                    activeKey: 'order_cjmSlave0List', // 当前显示页签
                    onChange: this.onTabChangedTablelist, // 选中页签改变
                    onTabClick: EMPTY_FN, // 点击页签
                    tablePanes: { // 页签信息
                        'order_cjmSlave0List'/*区域编码*/: {
                            key: 'order_cjmSlave0List', // 页签唯一标识
                            tab: '子实体0', // 页签显示名称
                            table: { // 子表表格
                                area: 'order_cjmSlave0List', // 区域编码
                                showIndex: true, // 是否显示行序号
                                showCheck: false, // 是否显示多选框
                                adaptionHeight: false, // 是否自适应浏览器高度
                                height: 200,
            ...
            card: { // 卡片页面
                form: { // 表单
                    area: 'order_cjmMasterForm', // 区域编码
                    onBeforeEvent: this.onBeforeEditForm, // 编辑前事件
                    onAfterEvent: this.onAfterEditForm, // 编辑后事件
                },
                formlist: { // 子表表格
                    'order_cjmSlave0Sub'/*区域编码*/: {
                        area: 'order_cjmSlave0Sub', // 区域编码
                        showIndex: true, // 是否显示行序号
                        showCheck: true, // 是否显示多选框
                        adaptionHeight: true, // 是否自适应浏览器高度
                        onAfterEvent: this.onAfterEditFormlist, // 编辑后事件
                        ...
                        onSelectedAll: this.onSelectedChangeFormlist, // 点击全选框
                        selectedChange: this.onSelectedChangeFormlist, // 选择框的选中行改变
                        modelSave: () => this.onFormlistModelSave('order_cjmSlave0Sub'), // 整单保存
                        tableHead: () => this.state.card.formlist['order_cjmSlave0Sub'].button.render(), // 肩部渲染，主要用于渲染肩部操作按钮，调用肩部按钮的渲染方法
                        button: { // 肩部按钮
                            area: 'order_cjmSlave0Sub', // 区域编码
                            onButtonClick: (props, btnCode) => this.onBtnClickFormlist(props, btnCode, 'order_cjmSlave0Sub'), // 点击按钮
                            render: () => { // 渲染方法
                                return (
                                    <div className="shoulder-definition-area">
                                        <div className="btn-group">
                                            {/*按钮*/}
                                            {this.props.button.createButtonApp(this.state.card.formlist['order_cjmSlave0Sub'].button)}
                                        </div>
                                    </div>
                                );
                            }
    ...
    render() {
        ...
        // 渲染列表
        const renderList = () => {
            let {search, table, tablelist} = list;
            let {tablePanes} = tablelist;
            ...
            return (
                <div className="nc-bill-list">
                    ...
                    <div className="nc-bill-table-area">
                        <NCTabs {...tablelist}>
                            {
                                (Object.values(tablePanes) || []).map(cfg => {
                                    return (
                                        <NCTabs.NCTabPane {...cfg}>
                                            {this.props.table.createSimpleTable(cfg.table.area, cfg.table)}
                                        </NCTabs.NCTabPane>
                                    );
                                })
                            }
        ...
        const renderCard = () => {
            ...
            let {'order_cjmSlave0Sub' : tableConfig, ...other} = newFormlist;
            return (
    ...
    // 查询列表子表数据
    // callback  回调，(data: 查询结果，格式参考后端Grid类) => {}
    listSlaveData = ({callback = EMPTY_FN} = {}) => {
        let {tablelist} = this.state.list;
        // 获取分页信息
        let pageInfo = this.props.table.getTablePageInfo(tablelist.activeKey);
        // 获取列表子表对应卡片子表区域编码
        // 因为列表子表没有实际模板，是页面初始化时复制的卡片子表的模板，所以是根据卡片子表区域编码进行的查询
        let areaId = tablelist.activeKey;
        let param = { // 组装参数
            pk: tablelist.tablelistRow.values[FIELDS.PRIMARYKEY].value, // 主实体主键
            formId: areaId, // 卡片子表区域编码
            pageInfo: pageInfo, // 分页信息
            pagecode: this.config.pagecode, // pagecode
        };
        ajax({
            url: URLS.listSlaveUrl,
            data: param,
            success: (res) => {
                let {data: {data} = {}} = res;
                callback(data);
    ...
    // 填充列表子表数据
    // data  子表数据，格式参考后端Grid类，为空则会清空
    // callback  回调，无参数
    fillSlaveData = ({data, callback = EMPTY_FN} = {}) => {
        let {list: {tablelist: {tablePanes}}} = this.state;
        // 遍历列表子表
        Object.keys(tablePanes).forEach(id => {
            // id是列表子表区域编码，获取对应卡片子表区域编码
            // 因为列表子表没有实际模板，是页面初始化时复制的卡片子表的模板，所以是根据卡片子表区域编码进行的查询
            let areaId = id;
            // 通过卡片子表区域编码获取子表数据
            let tableData = data && data[areaId] ? data[areaId] : {rows: []};
            // 设置列表子表数据
            this.props.table.setAllTableData(id, tableData);
        })
        setTimeout(callback);
    }
    ...
    afterLoadList = ({data, callback = EMPTY_FN} = {}) => {
        ...
        // 重置列表子表详情对应主表行
        this.state.list.tablelist.tablelistRow = data && data[table.area].rows && data[table.area].rows.length > 0 ? data[table.area].rows[0] : {};
        this.setState(this.state, () => {
            // 填充列表数据
            this.fillListData({data});

            let {tablelistRow} = this.state.list.tablelist;
            if (tablelistRow && tablelistRow.values) {
                // 调用子表查询
                this.listSlaveData({
                    callback: (data) => {
                        // 填充列表子表表格数据
                        this.fillSlaveData({data});
                    }
                });
            } else {
                // 清空列表子表数据
    ...
    beforePastCardTable = (tableId, row) => {
        switch (tableId) {
            case 'order_cjmSlave0Sub':
                row.values['pk_order_cjmslave0'] = {value: '', display: ''};
     ...
     * @param key  页签id
     */
    onTabChangedTablelist = (key) => {
        // 设置当前选中页签标识
        this.state.list.tablelist.activeKey = key;
        this.setState(this.state, () => {
            let {tablelistRow} = this.state.list.tablelist;
            // 判断是否选择了主表数据行
            if (!(tablelistRow && tablelistRow.values)) {
...
ApplicationPage = createPage({
    ...
    billinfo: {
        billtype: 'extcard',
        pagecode: '4004900H1_order_cjmMaster',
        headcode: 'order_cjmMasterForm',
        bodycode: ['order_cjmSlave0Sub']
    },
    orderOfHotKey: ['order_cjmMasterForm', 'order_cjmSlave0Sub']
})(ApplicationPage);
