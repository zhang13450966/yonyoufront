...
const delTreeNode = (tree, children, key, isContainChild) => {
    // //debugger
    Array.isArray(children) && children.forEach((data, index) => {
        if (data.key === key) {
            if (!isContainChild) {
...
/**
 * treeDataList 中是否包含keyid为key的元素
 * @param key
 * @param treeDataList
 * @returns {boolean}
 */
...
function addToRightList(checkedNodes, rightTree) {
    let newCheckedNodes = deepClone(checkedNodes);
    let currentNode = getNodeByKey(rightTree, this.currentNodeKey);
    if (currentNode && currentNode.isDir) {
        newCheckedNodes.forEach(item => {
            currentNode.children || (currentNode.children = []);
            !item.isDir && currentNode.children.push(item);  //右树中添加所有选中节点数据
        });
    } else {
        newCheckedNodes.forEach(item => {
            rightTree[0].children || (rightTree[0].children = []);
            !item.isDir && rightTree[0].children.push(item);  //右树中添加所有选中节点数据
        });
    }

}

function addToTreeIgnoreDir() {
