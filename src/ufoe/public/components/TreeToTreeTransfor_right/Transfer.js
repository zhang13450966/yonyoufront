...
export default class Transfer extends Component {
    ...
    moveNodes = (type) => {
        ...
        if (rightHalfCheckedKeys.length === 0) {  //当全部选中
            return $nccUtil.toast($appRoot.state.json['public_lang-000250'], $nccConst.spell.warning);/* 国际化处理： 请选择需要移动的表样*/
        }
        if (rightHalfCheckedKeys.length >= 3) {
            /*办选节点超过3个的时候，证明至少存在2个表样分类没有被全选，此时不支持移动*/
            $nccUtil.toast($appRoot.state.json['public_lang-000251'], $nccConst.spell.warning);/* 国际化处理： 不支持多个分组内的表样同时移动*/
            return;
    ...
    addStyle = () => {
        ...
        let title = $appRoot.state.json['public_lang-000253'];/* 国际化处理： 分组*/
        let refname = $appRoot.state.json['public_lang-000253'];/* 国际化处理： 分组*/
        let pid = rightTree[0].id;
        let name = $appRoot.state.json['public_lang-000253'];/* 国际化处理： 分组*/
        let isDir = true;
        let children = [];
        let typeObj = {
        ...
        return this.props.original ? (
            ...
            (
                <Treetransfer
                    ...
                    leftSelectDataLength={this.state.leftCheckedKeys.length}
                    rightSelectDataLength={this.state.rightCheckedKeys.length}
                    leftDataLength={this.leftDataList.length}
                    rightDataLength={this.rightDataList.length}
