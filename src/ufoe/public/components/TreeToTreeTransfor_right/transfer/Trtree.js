...
function createTree({
    ...
    const loop = data => data.map((item) => {
        ...
        if (this.state.editKey === item.key) {
            inputTitle = <input type="text" id="itemKey" defaultValue={item.name}
        ...
        if (this.state.isHover === item.key && item.isDir && !item.isRoot) {
            editIcon = <i type="uf-code" className="icon iconfont icon-bianji edit-icon"
                          onClick={(e) => this.editRender(item)}/>;
        }

        //删除图标
        if (this.state.isHover === item.key && item.isDir && !item.isRoot) {
            deleIcon = <i type="uf-code" className="icon iconfont icon-shanchu delete-ico"
                          onClick={(e) => this.deleteNode(item)}/>;
        }
    ...
    const onSelectNode = (selectedKeys, e) => {
        ...
        let hasExpened = false;
        let expandedKeys = !this.state.rightExpandedKeys ? [] : this.state.rightExpandedKeys;
        let eventKey = e.node.props.eventKey;
        this.state.rightExpandedKeys.forEach((value)=>{
            if(value === eventKey){
                hasExpened = true;
                let index = expandedKeys.indexOf(value);
