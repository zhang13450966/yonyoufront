...
class PublishModal extends Component {
	...
	onHandleClick = (way) => {
		...
		switch (way) {
			...
			case 'finish':
				...
				if (!pageData.parentMenuCode || (pageData.parentMenuCode && pageData.parentMenuCode.length !== 6)){
					return toast({
						content: $appRoot.state.json['public_lang-000294'],/* 国际化处理： 菜单注册树节点必须选第三层*/
						color: 'warning'
					}); 
				}	
	...
	onSelectEveSecond = (code, data, a, b) => {
		...
		if(code && code.length !== 6){
			$nccPlatform.toast({ color: 'warning', content: $appRoot.state.json['public_lang-000294']});/* 国际化处理： 菜单注册树节点必须选第三层*/
		}
		let pos = b.node.props.pos;
		pos = pos.split('-');
		if (pos.length == 4 && !data.isleaf) {
