...
export default class Transfer extends Component {
    constructor(props) {
        super(props);
        this.props = props;
        window.TreeToTree = this
        let leftRoot = _.get(props.leftTreeData[0],'key')
        let leftExpandedKeys = leftRoot ? [leftRoot] : []
        let rightRoot = _.get(props.rightTreeData[0],'key')
        let rightExpandedKeys = rightRoot ? [rightRoot] : []        
        this.state = {
            ...
            leftExpandedKeys: leftExpandedKeys,  //左侧展开的节点？
            ...
            leftSearchTree: [], // 左侧搜索后的树
            rightSearchValue: "",
            isLeftSearch: false,
            isRightSearch: false,
            // rightSearchTree: this.props.rightTreeData || [],
            rightSearchTree: [],// 右侧搜索后的树
        ...
        this.leftCanTransferList = []
        this.leftTreeAllFlatData = [] // 左侧树平铺list数据
        this.leftDataMap = new Map() // 左侧树平铺Map               
        let {leftTreeAllFlatData,dataMap} = this.treeToArray(this.props.leftTreeData)
        this.leftTreeAllFlatData = leftTreeAllFlatData // 左侧树平铺list数据
        this.leftDataMap = dataMap // 左侧树平铺Map       
        this.filterLeftFlatData = []
        this.hideLeftCheckedKeys = []
        this.hideRightCheckedKeys = []
    ...
    componentWillReceiveProps(nextProps) { 
        let leftRoot = _.get(nextProps.leftTreeData[0],'key')
        let leftExpandedKeys = leftRoot ? [leftRoot] : []
        let rightRoot = _.get(nextProps.rightTreeData[0],'key')
        let rightExpandedKeys = rightRoot ? [rightRoot] : []
        this.setState(prevState => ({
            ...
            leftExpandedKeys, // 左侧展开节点
            rightExpandedKeys, // 右侧展开节点
     ...
     */
    toLeft = () => {
        let checkedKeys = this.state.rightCheckedKeys;
        let halfCheckedKeys = this.state.rightHalfCheckedKeys;
        let moveNodes = [];
        if (checkedKeys.length == 0) {
            ...
            let {leftSearchValue,rightSearchValue} = this.state     
            this.setState({
                rightCheckedKeys: [],
                rightHalfCheckedKeys: [],
                leftCheckedKeys: [],
                ...
                isLeftSearch: !!leftSearchValue,
                isRightSearch: !!rightSearchValue
            });
            this.leftDataList = [];
            this.rightDataList = [];
            // this.isLeftSearch = false;
     ...
     */
    toRight = () => {
        let checkedKeys = this.state.leftCheckedKeys;  //获取选中的节点
        ...
        let halfCheckedKeys = this.state.leftHalfCheckedKeys;  //获取半选节点
        ...
        if (checkedKeys.length === 0) {
            ...
            // rightTree 在上一行代码中形成的树层级不准确，用下面代码重新生成
            this.state.rightTreeData = this.getRightTreeData(checkedKeys)
            ...
            let {leftSearchValue,rightSearchValue,rightExpandedKeys} = this.state
            let rightRootKey = _.get(this.state.rightTreeData,'[0].key','')
            if (!rightExpandedKeys.includes(rightRootKey)) {
                rightExpandedKeys.push(rightRootKey)
            }
            this.setState({
                leftCheckedKeys: [],
                leftHalfCheckedKeys: [],
    ...
    delTreeNodeIgnoreDir = (nodes, treeid) => {
        let {isRightSearch,rightSearchTree,rightTreeData} = this.state
        nodes.forEach((item, index) => {
    ...
    onCheck = (treeId, checkedKeys, halfCheckedKeys, directExceptParentNodeKeys) => {
        let {hideLeftCheckedKeys,hideRightCheckedKeys} = this
        let {leftSearchValue,rightSearchValue} = this.state
        ...
        let leftCheckedKeys = [...checkedKeys,...hideLeftCheckedKeys]
        let rightCheckedKeys = [...checkedKeys,...hideRightCheckedKeys]
        if (treeId == this.leftTreeId) {
            this.setState({
                leftCheckedKeys,
    ...
    changeReportStyle = (style) => {
        ...
        if(_.isEmpty(rightCheckedKeys)){
            toast({content: $appRoot.state.json["public_lang-000293"], color: "warning"});/* 国际化处理： 请选择表样*/
        }
    ...
    treeToArray = (treeData, { pid, leafKeys = new Set(), treeAllFlatData = [], dataMap = new Map() } = {}) => {
        let { rowChildren = 'children', rowKey = 'key', rowTitle = 'title' } = this.props;
    ...
    // 根据左侧树勾选节点，反查父节点获得树数据作为右侧树结构
    getRightTreeData = (checkedKeys) => {
        let {leftDataList,rightDataList} = this
        let rowSet = new Set();
        let allResultRow = [];        
        let dataMap = new Map()        
        let leftDataListCopy = leftDataList.map(item => {return {...item,...item.node,children:[]}})
        let toRightLeafList = leftDataListCopy.filter(item => item.isleaf || item.isReport || item.isFormula || item.isSchema).filter(item => checkedKeys.includes(item.key))
        let rightDataListCopy = rightDataList.map(item => {return {...item,...item.node,children:[]}})
        let rightLeafList = rightDataListCopy.filter(item => item.isleaf || item.isReport || item.isFormula || item.isSchema);
        [...leftDataListCopy,...rightDataListCopy].forEach(item => {
            dataMap.set(item.key,item)
        });
         // 反查父节点
         [...toRightLeafList,...rightLeafList].forEach(node => {
            let parentRow = [];
        ...
        return this.listToTree(allResultRow)
    }
    // 根据左侧树剩余可穿的节点，反查父节点获得树数据
    treeFilter = (leftTreeAllFlatData,rightTreeAllFlatData) => {
        // 报表数据中心的审核条件用此穿梭框，判断是叶子节点的条件不一样,有isReport和isFormula
        let rightLeafList = new Set(rightTreeAllFlatData.filter(item => item.isleaf || item.isReport || item.isFormula || item.isSchema).map(item => item.key))
        let leftLeafNodes = leftTreeAllFlatData.filter(item => item.isleaf  || item.isReport || item.isFormula || item.isSchema)
        leftLeafNodes = leftLeafNodes.filter(item => !rightLeafList.has(item.key))
        let rowSet = new Set();
        let allResultRow = [];        
        let dataMap = new Map()
        leftTreeAllFlatData.forEach(item => {
            dataMap.set(item.key,item)
        })        
        // 反查父节点
        leftLeafNodes.forEach(node => {
            let parentRow = [];
            let parentNode = dataMap.get(node.pid);
            if (!rowSet.has(node.key)) {
                rowSet.add(node.key);
     ...
     */
    leftArea = () => {
        let {leftDataMap} = this
        let {leftExpandedKeys,isLeftSearch,leftSearchTree,leftTreeData,selectType,leftCheckedKeys,leftTreeConfig,leftSearchValue,rightTreeData} = this.state
        let data = isLeftSearch ? leftSearchTree : leftTreeData;
        let {treeAllFlatData:leftTreeAllFlatData} = this.treeToArray(data)
        let {treeAllFlatData:rightTreeAllFlatData} = this.treeToArray(rightTreeData)
        let filterLeftFlatData = this.treeFilter(leftTreeAllFlatData,rightTreeAllFlatData)
        this.filterLeftFlatData = filterLeftFlatData
        let finalLeftTreeData = this.listToTree(filterLeftFlatData)
        return this.TransferTreeMethods.createTree.call(this, {
            treeId: this.leftTreeId,
            data: finalLeftTreeData,
            onExpand: this.onExpand,
            onCheck: this.onCheck,
            searchMode: isLeftSearch,
            selectType: selectType,
            cacheTree: leftTreeData,
            clearCheckedKeys: this.clearCheckedKeys,
            searchValue: leftSearchValue,
            checkedKeys: leftCheckedKeys,
            expandedKeys: leftExpandedKeys,
            autoExpandParent: false,
            otherConfig: leftTreeConfig,
     ...
     */
    rightArea = () => {
        let {rightExpandedKeys,isRightSearch,rightSearchTree,rightTreeData,rightSearchValue,rightCheckedKeys,selectType,rightTreeConfig,isDraggable} = this.state
        let data = isRightSearch ? rightSearchTree : rightTreeData;
        return this.TransferTreeMethods.createTree.call(this, {
            ...
            searchMode: isRightSearch,
            cacheTree: rightTreeData,
            selectType: selectType,
            autoExpandParent: false,
            onExpand: this.onExpand,
            onCheck: this.onCheck,
            clearCheckedKeys: this.clearCheckedKeys,
            otherConfig: rightTreeConfig,
            fixed: this.props.rightFixed || this.props.leftFixed ? this.props.leftFixed ? this.leftTreeId : this.rightTreeId : "",
    ...
    addStyle = () => {
        ...
        this.setState({
           ...
           let treeDom= document.querySelector('.right-area .tree-hotkeys-wrapper')            
            treeDom.scrollTop = treeDom.scrollHeight - treeDom.clientHeight
        });
    };
    ...
    toLeft2 = () => {
        ...
        if (!checkedKeys.length) {
            ...
            let that = this;
            setTimeout(()=>{
                that.state.leftSearchValue && that.leftSearch(that.state.leftSearchValue);
            })
        }
    };
    ...
    allToLeft2 = () => {
        let rightTree = this.TransferTreeMethods.getTreeDataById(this.rightTreeId);
        let checkedKeys = this.TransferTreeMethods.getAllNodeKeys(rightTree);
        this.state.rightCheckedKeys = checkedKeys.reverse();
        this.toLeft2();
    };
    ...
    allToRight2 = () => {
        let leftTree = this.TransferTreeMethods.getTreeDataById(this.leftTreeId);
        let checkedKeys = this.TransferTreeMethods.getAllNodeKeys(leftTree);
        this.state.leftCheckedKeys = checkedKeys.reverse();
        this.toRight2();
    };
    ...
    onRightCheckAllChange = flag => {
        let {isRightSearch,rightSearchTree,rightTreeData} = this.state
        let data = isRightSearch ? rightSearchTree : rightTreeData;        
        let {treeAllFlatData:rightTreeAllFlatData} = this.treeToArray(data)
        if (flag) {
            const rightKeys = rightTreeAllFlatData.reduce((did, cur) => {
                did.push(cur.key);
                return did;
            }, []);
            this.setState({
    ...
    onLeftCheckAllChange = flag => {
        let {isLeftSearch,leftSearchTree,leftTreeData} = this.state
        let data = isLeftSearch ? leftSearchTree : leftTreeData;
        let {treeAllFlatData:leftTreeAllFlatData} = this.treeToArray(data)        
        if (flag) {
            const leftKeys = leftTreeAllFlatData.reduce((did, cur) => {
                did.push(cur.key);
                return did;
            }, []);
            this.setState({
    ...
    filterNotDirNodes = (dataList, LeftOrRight) => {
        let {leftSearchValue,leftSearchTree,rightSearchValue,rightSearchTree} = this.state
        let allKeys = []
        let leftSearchTreeList = []
        let rightSearchTreeList = []
        if (leftSearchValue && LeftOrRight == 'left') {
            this.generateList(leftSearchTree,leftSearchTreeList)
            allKeys = leftSearchTreeList.map(item => item.keyid)
        } else if (rightSearchValue && LeftOrRight == 'right') {
            this.generateList(rightSearchTree,rightSearchTreeList)
            allKeys = rightSearchTreeList.map(item => item.keyid)
        } else {
            allKeys = dataList.map(item => item.keyid);
        }
        return this.filterNotDirKeys(allKeys, LeftOrRight);
    };

    render() {
        let leftArea = this.leftArea.bind(this)
        // 计算leftSelectDataLength的值
        let {leftExpandedKeys,isLeftSearch,leftSearchTree,leftTreeData,isRightSearch,rightSearchTree,rightTreeData,leftCheckedKeys,rightCheckedKeys} = this.state
        let leftData = isLeftSearch ? leftSearchTree : leftTreeData;
        let {treeAllFlatData:leftTreeAllFlatData} = this.treeToArray(leftData)
        let {treeAllFlatData:rightTreeAllFlatData} = this.treeToArray(rightTreeData)
        let filterLeftFlatData = this.treeFilter(leftTreeAllFlatData,rightTreeAllFlatData)
        this.filterLeftFlatData = filterLeftFlatData
        let leftSelectDataLength = this.filterNotDirKeys(leftCheckedKeys.filter(key => filterLeftFlatData.find(ele => ele.key === key)), "left").length
        this.hideLeftCheckedKeys = isLeftSearch ? [...new Set([...this.hideLeftCheckedKeys,...leftCheckedKeys.filter(key => !filterLeftFlatData.map(ele => ele.key).includes(key))])] : []
        // 计算rightSelectDataLength的值
        let {} = this.state
        let rightData = isRightSearch ? rightSearchTree : rightTreeData;        
        let {treeAllFlatData:rightTreeAllFlatData2} = this.treeToArray(rightData)
        let rightSelectDataLength = this.filterNotDirKeys(rightCheckedKeys.filter(key => rightTreeAllFlatData2.find(ele => ele.key === key)), "right").length
        this.hideRightCheckedKeys = isRightSearch ? [...new Set([...this.hideRightCheckedKeys,...rightCheckedKeys.filter(key => !rightTreeAllFlatData2.map(ele => ele.key).includes(key))])] : []
        //第二个是常用的树
        return this.props.original ? (
                <Treetransfer
                    ...
                    leftChecked={this.state.leftCheckedKeys.length > 0 && this.state.leftCheckedKeys.length === this.leftDataList.length}
                    ...
                    rightChecked={this.state.rightCheckedKeys.length > 0 && this.state.rightCheckedKeys.length === this.rightDataList.length}  //右侧肩部的复选框状态
                    ...
                    leftSelectDataLength={leftSelectDataLength}
                    rightSelectDataLength={rightSelectDataLength}
