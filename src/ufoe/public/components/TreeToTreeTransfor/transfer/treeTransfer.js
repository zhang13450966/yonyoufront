...
class Treetransfer extends Component {
    constructor(props) {
        ...
        window.TreeToTreeTransfer = this
    }
    ...
    render() {
        let {leftSelectDataLength,leftDataLength,rightSelectDataLength,rightDataLength} = this.props  
        let {leftDataList,rightDataList} = this.props.Transfer  
        let {leftTreeData,rightTreeData,leftCheckedKeys,leftHalfCheckedKeys,rightHalfCheckedKeys,rightCheckedKeys,rightSearchTree} = this.props.Transfer.state  
        let leftCanTransferList = leftDataList.filter(item => item.node.isleaf)
        ...
        return (
            <NCDiv fieldid="transfer"
                        ...
                        <div className='content-box nc-theme-area-split-bc nc-theme-transfer-wrap-bgc'>
                            <div id='myTitle'>
                                <div style={{float: "left"}}>
                                    ...
                                    />
                                    {`${leftSelectDataLength}/${leftDataLength}`}{$appRoot.state.json['public_lang-000132']}{/* 国际化处理： 条数据*/}
                                </div>
                ...
                {/*中间按钮区域*/}
                {this.props.isHideLeft ? null : <MoveButton
                    hiddenAllMoveBtns = {this.state.hiddenAllMoveBtns}
                    left={{
                        text: '',
                        active: rightCheckedKeys.length > 0 ? true : false,
                        move: this.props.toLeft,
                    }}
                    right={{
                        text: '',
                        active: leftCheckedKeys.length > 0 ? true : false,
                        move: this.props.toRight,
                    }}
                    allLeft={{
                        text: '',
                        active: rightDataLength > 0 , // ||  (this.props.rightFixed && !this.props.allToLeft_without_merge || this.state.disableBtns ? false : true) 
                        move: this.props.allToLeft,
                    }}
                    allRight={{
                        text: '',
                        active: leftDataLength > 0 ? true : false,
                        move: this.props.allToRight,
                    }}
                    langJson={{}} // todo
                    ...
                    <div className="content-box nc-theme-area-split-bc nc-theme-transfer-wrap-bgc">
                        <div id='myTitle'>
                            <div style={{float: "left"}}>
                                ...
                                />
                                {`${rightSelectDataLength}/${rightDataLength}`}{$appRoot.state.json['public_lang-000132']}{/* 国际化处理： 条数据*/}
                            </div>
                            <div style={{float: "right"}}>
                ...
                {!this.props.original && (<div className='button-area'>
                    <div className="opr-botton">
                        <NCButton fieldid='000237' id="personalBtn" colors="primary"
                    ...
                    <div className='group_one'>
                        <div className="opr-botton">
                            <NCButton fieldid='000027' id="personalBtn_up" className="nc-theme-btn-secondary" disabled={this.props.moveFlag}
                                    onClick={e => this.props.moveNodes("up")}>{$appRoot.state.json['public_lang-000027']}</NCButton>{/* 国际化处理： 上移*/}
                        </div>
                        <div className="opr-botton">
                            <NCButton fieldid='000028' id="personalBtn_down" className="nc-theme-btn-secondary" disabled={this.props.moveFlag}
                                    onClick={e => this.props.moveNodes("down")}>{$appRoot.state.json['public_lang-000028']}</NCButton>{/* 国际化处理： 下移*/}
                        </div>
                    </div>
                    <div className='group_two'>
                        <div className="opr-botton">
                            <NCButton fieldid='personalBtnOne' id="personalBtnOne" className="nc-theme-btn-secondary"
                                    onClick={e => this.props.changeReportStyle("reportedRecorded")}>{$appRoot.state.json['public_lang-000238']}</NCButton>{/* 国际化处理： 必报必录*/}
                        </div>
                        <div className="opr-botton">
                            <NCButton fieldid='personalBtnTwo' id="personalBtnTwo" className="nc-theme-btn-secondary"
                                    onClick={e => this.props.changeReportStyle("reported")}>{$appRoot.state.json['public_lang-000239']}</NCButton>{/* 国际化处理： 必报非必录*/}
                        </div>
                        <div className="opr-botton">
                            <NCButton fieldid='personalBtnThree'  id="personalBtnThree" className="nc-theme-btn-secondary"
