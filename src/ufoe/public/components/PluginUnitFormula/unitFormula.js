...
import ExampleManagerCom from "./exampleManagerCom";
...
class UnitFormula extends Component {
    ...
    constructor(props) {
        ...
        this.backProcessorMapping = new Map([
            ...
            // 6
            [   
                "nc.ui.hbbb.dxrelation.formula.refprocessor.ReportDataSourceProcessor",
                {
                    type: "select",
                    dataType: "number",
                }
            ...
            // 7
            [   
                "nc.ui.hbbb.dxrelation.formula.refprocessor.INTRReportDataSourceProcessor",
                {
                    type: "select",
                    dataType: "number",
                }
            ...
            // 8
            [   
                "nc.ui.hbbb.dxrelation.formula.refprocessor.UAPDataSourceProcessor",
                {
                    type: "select",
                    dataType: "number",
                }
            ...
            // 9 借贷方向
            [   
                "nc.ui.hbbb.dxrelation.formula.refprocessor.AmountDirectionProcessor",
                {
                    type: "select",
                    dataType: "number",
                }
    ...
    createOptBtns = btnConfig => {
        ...
        htmls = [...btnConfig.values()].map((config, index) => {
            ...
            return (
                <NCButton fieldid={key} onClick={this.handelSetName} key={index} data-name={key} className="opt-btn">
                    {name}
                </NCButton>
            );
        });
    ...
    handelSetName = e => {
        ...
        // 平台NCButton组件内多了span标签,就获取不到button元素上的dataset了
                    ...
                    >
                        {
                            tabConfig.map(tab => {
                                const { name, key } = tab;
                                return (
                                    <NCTabPane tab={name} key={key} fieldid={key}>
    ...
    generatorTabs = () => {
        ...
        const { dataSource,demoContent,defaultSelectedKeys } = this.state;

        const { DragWidthCom } = this.props;       
        let exampleValue = "";
        if ($appRoot.UnitFormula.state.dataSource.length != 0) {
            exampleValue = JSON.stringify($appRoot.UnitFormula.state);
        }
        if (!dataSource.length) { return null }
        /** 填充示例列表 */
        if (searchListState != defaultSelectedKeys) {
            this.getDemoTableData();
        }
        searchListState = defaultSelectedKeys;
        return (
            <div className="tabs-left" id="fu_me_tabs_left">
                <NCTabs
                    tabBarStyle="primary"
                    className="ys-tabs-bar"
                        ...
                        dataSource.map(tab => {
                            ...
                            return (
                                <NCTabPane tab={tabName} key={key} fieldid={key}>
                                    <div className="tab-pane-wrapper exampleValue">
                                    <input type = 'hidden' value = {exampleValue} />
                                        <DragWidthCom
                                            // 左树区域
                                            defLeftWid="40%"
                                            leftMinWid="140px"
        ...
        return (
            <div className="fun-container">
                <NCMenu
                    ...
                    onSelect={({ item, key, selectedKeys, keyPath }) => {
                        this.currentFunction = null;
                        const currentFuncInfo = item.props.funcInfo;
                        // const subMenuKey = item.props.subMenuKey;
                        const subMenuKey = keyPath[keyPath.length-1];
                ...
                >
                    {
                        funcTypes.map(funcType => {
                            ...
                            return (
                                <SubMenu title={funcTypeName} key={funcTypeName} fieldid={funcTypeName}>
                                    {
                                        funcs.map(func => {
    ...
    generatorParameterTable = (currentFuncInfo, reftype) => {
        ...
        return paramNames.map((paramName, index) => {
            ...
            if (paramRefList[index]) {
                ...
                // 时间属性 、
                if (paramKey.indexOf(".do")>-1 || paramKey === "/nccloud/ufoc/refer/inner/HbProjectRef/index" || paramKey === "/nccloud/uapbd/refer/fiacc/AccountDefaultGridTreeRef/index") {
                    return this.generatorParameterItem({
    ...
    generatorParameterItem = (config, paramIndex, options = []) => {
        ...
        const inputItem = (
            <div className="parameter-item">
                ...
                <div className="parameter-item-put" fieldid={funcCode+label}>
                    <NCInput
                        key={funcCode}
        ...
        const selectItem = (
            <div className="parameter-item">
                ...
                <div className="parameter-item-put" fieldid={funcCode+label}>
                    ...
                    >
                        {
                            options.map(option => <NCOption fieldid={option.value} key={option.value} value={option.value}>{option.display}</NCOption>)
                        }
                    </NCSelect>
                </div>
            </div>
        ...
        const referItem = (
            <div className="parameter-item" fieldid={itemValue+label}>
                <span className="parameter-label">{label}</span>
                    ...
                    <div fieldid={itemValue+label}>
                        <NCIcon
                            onClick={() => {
                                ...
                                if(paramKey === "ufoe/exports/components/Mselect/index"){ //参照类型指标选取不进行关键字校验
                                    sonCconfig["isCheckKeyWord"] = false
                                }
        ...
        let styleVal = {};
        if (label.indexOf('FBM') != -1 || label.indexOf('GLA') != -1 ) {
            styleVal = {width: '100px',right: '60%'};
        }
        // 常规参照之 外部门参照
        const externalItem = (
            <div className="parameter-item" fieldid={itemValue+label}>
                <span className="parameter-label" style={styleVal}>{label}</span>
                <div className="parameter-item-put parameter-item-refer">
                    <div className="parameter-item-refer-container" title={itemValue}>{itemValue}</div>
                    <div fieldid={itemValue+label}>
                        <GuidButton
        ...
        const platformItem = () => {
            ...
            return (
                <div className="parameter-item" fieldid={label}>
                    <span className="parameter-label" fieldid={label}>{label}</span>
                    <div className="parameter-item-page-refer">
                        {
                            creatRefer({
                                refName: label,
                                ...
                                onChange: (value) => {
                                    ...
                                    // }
                                    let referType = null;
                                    if(paramKey === "/nccloud/ufoc/refer/inner/HbProjectRef/index" || paramKey === "/nccloud/uapbd/refer/fiacc/AccountDefaultGridTreeRef/index"){
                                        if(_.isEmpty(cloneValue)){
                                            referType = {
                                                str: ''
                                            }
                                        }else{
                                            referType = {
                                                str: `'${cloneValue.refname}/${cloneValue.refcode}'`
                                            }
                                        }
                                    }else{
                                        referType = null;
        ...
        const accPeriodUFOERefItem = () => {
            ...
            return (
                <div className="parameter-item parameter-item-unit-formula" fieldid={label}>
                    <span className="parameter-label" fieldid={label}>{label}</span>
                    {
    ...
    onInsert = (value, referType) => {
        ...
        const { functionParameters } = this.state;
        //console.log("参照回调", value, this.saveReferparamIndex, functionParameters);
        if (str == undefined || str == "'undefined'" || str == 'undefined') {
            str = '';
        }
        functionParameters[this.saveReferparamIndex] = str;
    ...
    /** 获取当前选中函数的示例 */
    getDemoTableData = async () => {
        let metaEmpty = this.props.meta.getMeta();// 获取列表已存在的模板
        if(!metaEmpty.demoTable){
            //加载模板,添加到meta中
            this.props.meta.addMeta(examMeta('useTableTemp'));
        }
        let treeNode = $appRoot.UnitFormula.state.defaultSelectedKeys;
        if (!treeNode || treeNode.length == 0 ) {
            // treeNode = document.querySelectorAll('.fun-container .u-menu-item.u-menu-item-selected span.func-code')[0].innerText;
            treeNode = this.state.defaultSelectedKeys;
        }
        let res = await this.getTableDataByTreeNode(treeNode,'true');
        let tableData = [];
        if (res.data && res.data.length != 0) {
            tableData = this.dataDealDefault(res.data[Object.keys(res.data)[1]].rows);
            this.state.demoContent = tableData[0].values.demo_content.value;
        }else {
            this.state.demoContent = "";
        }
        const data = {
            rows: tableData
        }
        this.props.editTable.setTableData("demoTable", data); 
        this.addOperationColumn();
        if ($appRoot.UnitFormula.state.defaultSelectedKeys && $appRoot.UnitFormula.state.defaultSelectedKeys.length > 0) {
            $appRoot.UnitFormula.setState({});
        } else {
           this.setState({});
        }
    }
    ...
    addOperationColumn = () => {
        ...
        meta.demoTable.items.forEach(element => {
            if (element.attrcode == 'opr') {
                element.itemtype = 'customer';
                element.width = '120px'
                element.render = (text, record, index) => {
                    let billNoClick = () => {
                        let text = record.values.demo_content.value;
                        this.handlePushStr(text);
                    };
                    let content = $appRoot.state.json['public_lang-000274'];/* 国际化处理： 使用*/
                    return (
                        <span key={index} onClick={billNoClick} style={{ color: '#007ace', cursor: 'pointer', marginRight:'8px' }}>
                            {content}
                        </span>
                    );
                }
    ...
    getTableDataByTreeNode = (treeNode,isLeaf) => {
        let funcname = [];
        funcname.push(treeNode);
        return new Promise(resolve => {
            let url =CONSTANT.getEMTreeData;
            ajax({
                url: url,
                data: { "func_name": funcname,"isleaf": isLeaf},
                success: res => {
                    resolve(res);
                },
            });
    ...
    generatorGuideAndOperation = () => {
        ...
        if (this.textObj[funcCode]) {
            ...
            rdom = <div className="guide">
                ...
                <div className="guide-bottom demoTableList" style={{width: '565px'}}>
                    {
                        this.props.editTable.createEditTable('demoTable', {
                        height:60,
    ...
    guideFunctionBtnConfig = (type = "") => {
        if (type === "conditions") {
            ...
            let btnConfig = [
                ...
                { key: "exampleManager", name: $appRoot.state.json['public_lang-000262'], onClick: () => { this.handleGuideExampleManager("guideCause", this.currentFunction) } },/* 国际化处理： 示例管理器*/
            ];
    ...
    handleGuideUse = (area, validateString) => {
        /** 按照规则填充函数内容 */
        let demoContent = this.state.demoContent;
        if (this.state.defaultSelectedKeys + "()" == validateString && demoContent.length > 0) {
            this.handlePushStr(demoContent);
        } else {
    ...
    handleValidate = (area, validateString,textareaCause) => {
        ...
        return new Promise(resolve => {
            ...
            if (validateString == '') {
                resolve(true);
                // 新需求调整，验证内容为空提示：条件为空值，请填写公式内容
                this.setState({ [area]: $appRoot.state.json['public_lang-000270'] });/* 国际化处理： 验证成功*/
            } else {
     ...
     *示例管理器 
     */
    handleGuideExampleManager = (info) => {
        if ($appRoot.UnitFormula.state.dataSource.length == 0 && this.state.dataSource.length == 0) {
            let exampleValue = JSON.parse(document.querySelectorAll('.exampleValue input')[0].value);
            $appRoot.UnitFormula.state = exampleValue;
        } else {
            $appRoot.UnitFormula = this;
        }
    ...
    generatorFunctionDesc = () => {
        ...
        return (
            <div className={txtVakye ? "fun-desc-wrapper" : "fun-desc-wrapper flex-container"} style={{height: '100%'}}>
                <div className="fun-desc-top">
                    <span>{$appRoot.state.json['public_lang-000190']} : </span>{/* 国际化处理： 函数功能*/}
                    <span>{simpDesc}</span>
                </div>
                < div className="fun-desc-table-wrapper flex-container" >
                    {
                        txtVakye?<p style={{ whiteSpace:'pre', overflow:'auto'}}>
                            {txtVakye}
                        </p>
        ...
        return (
            <div style={style} className="uf-wrapper mendege-uf-wrapper">
                <div className="uf-container">
                    ...
                    {createModal("example-manager", {
                        title: $appRoot.state.json['public_lang-000255'],/* 国际化处理： 示例管理器*/
                        // resizable: false,
                        className: "example-manager",
                        // content: this.generatorFunctionExampleManager(),
                        content:  <ExampleManagerCom  {...this.props}/>,
                        // zIndex: 300,
                        // bodyHeight: '600px',
                        size:"xlg",
                        noFooter: true,
                        closeModalEve: function(eve) {
                            searchListState = "";
                        }
                    })}
    ...
    return class Son extends React.Component {
        ...
        showModel = async (config = {}, modeConfig = {}) => {
            ...
            if (config.isConditionsReference) {
                ...
                if (!this.queryRptMngFormula || config.mid == 'dxrelation_card') {
                    let url = CONSTANT.queryRptMngFormula, checkPara={};
                    // 抵销模板公式不能使用缓存
                    if (config.url) {
                        url = config.url;
                        if(config.mid == 'dxrelation_card'){
                            let dxModelType = $appRoot.props.form.getFormItemsValue('dxrelationHead','type').value;
                            checkPara = {
                                dxModelType: dxModelType || ''
                            }
                        }
                    }
                    vres = await $nccUtil.promiseAjax(url, checkPara);
        ...
        */
                                    ...
                                    onClick={() => {
                                        ...
                                        that.props.unitFormula.showModel({
                                            ...
                                            beSureBtnClick: (data) => {
                                                ...
                                                // this.setState({twoTableState});
                                                // 调整及抵销使用的是cardTable
                                                if(unitFormulaId == 'dxrelation_card' || unitFormulaId == 'auditProgram_card'){
                                                    that.props.cardTable.setValByKeyAndIndex(id, index, attrCode, { value: data, display: data });
                                                }else{
        ...
        render() {
            ...
            let { title = title, mid = "formula_editor", Content = null, buttons = null,getTitle = () => {} } = configBox;
