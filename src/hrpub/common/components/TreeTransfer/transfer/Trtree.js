		...
		const loop = data => data.map((item) => {
			...
			if( item.key === undefined){
				//  console.log('error_item', item);
				 console.error(`${this.state.json['jf6005-000048']}，${this.state.json['jf6005-000049']}， ${this.state.json['jf6005-000050']}key${this.state.json['jf6005-000051']}，${this.state.json['jf6005-000052']}`);/* 国际化处理： 树组件,数据格式错误,缺少,字段,请检查数据格式*/
				 return false
			}
			if (item.children) {
				return (
					<NCTreeNode key={item.key} title={searchValue=="" ? iconTitle : title} disableCheckbox = {item.disabled ? true : false}>
						{loop(item.children)}
					</NCTreeNode>
				);
			}
			return <NCTreeNode key={item.key}  title={searchValue=="" ? iconTitle : title} disableCheckbox = { item.disabled ? true : false}/>;
		});
				...
				checkedKeys.forEach((itemKey, index)=> {
					...
					if(itemNodeInB.children && itemNodeInB.children.length > 0){
						...
						if(allNodeKeysInA.length == allNodeKeysInB.length && arrayBIsContaindA(allNodeKeysInA, allNodeKeysInB)){
							// 相等
							console.log(this.state.json['jf6005-000045'], itemKey)/* 国际化处理： 相等*/
							newCheckedKeys.push(itemKey);
						}else if(noneAInB(allNodeKeysInA, allNodeKeysInB)){
							console.log(this.state.json['jf6005-000046'])/* 国际化处理： 不包含*/
							// 不包含
						}else{
							console.log(this.state.json['jf6005-000047'], itemKey)/* 国际化处理： 包含部分*/
							// 包含部分
