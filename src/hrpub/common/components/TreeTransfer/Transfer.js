...
export default class Transfer extends Component{
    constructor(props) {
        ...
        this.state = {
            ...
            rightTreeData:  this.setRightInitData(this.props.rightTreeData)||[],//右树初始化数据
            leftExpandedKeys: [],
            rightExpandedKeys: [],
            leftCheckedKeys: [],
            leftHalfCheckedKeys: [],
    ...
    //添加右侧初始化数据禁用标识
    setRightInitData = ( rightData )=>{
        if( Array.isArray && this.props.rightFixed){
            rightData.map((item, index) => {
                item.disabled = true;
                this.setRightInitData(item.children);
            })
        }
        return rightData;
    }
    ...
    leftSearch = (searchValue) => {
        ...
        leafNodes.forEach(item => {
            let hasPar = this.TransferTreeMethods.getNodeByKey(this.props.leftTreeData, item.pid);
        ...
        if(this.state.treeType == 'VRFusion'){
            filterSearchNodes = searchNodes.filter(searchNode => {
                ...
                let virtualNode = this.TransferTreeMethods.getNodeByKey(this.props.leftTreeData, typeId);
    ...
    rightSearch = (searchValue) => {
        ...
        leafNodes.forEach(item => {
            let hasPar = this.TransferTreeMethods.getNodeByKey(this.props.rightTreeData, item.pid);
            if(this.state.treeType == 'VRFusion'){
                let typeId = item.nodeData.typeId;
                if(!hasPar){
                    if(expandedKeys.includes(typeId)){
        ...
        if(this.state.treeType == 'VRFusion'){
            filterSearchNodes = searchNodes.filter(searchNode => {
                let typeId = searchNode.nodeData.typeId;
                let virtualNode = this.TransferTreeMethods.getNodeByKey(this.props.rightTreeData, typeId);
                if(virtualNode && typeId != searchNode.id){
                    if(!virtualParMap[virtualNode.id]){
                        virtualNode.children=[];
                        virtualNode.children.push(searchNode);
