...
import getAttachments from '../ApproveOperationBtns/getAttachment.js';
require('./index.less');
const { NCCheckbox, NCButton, NCModal, NCTooltip } = base;
...
export default class ApproveComment extends Component {
    ...
    render() {
        ...
        let tips = {
            copy: <span className="nc-theme-sp-common-font-c">{json['index_014']}</span>,
            track: <span className="nc-theme-sp-common-font-c">{json['index_015']}</span>
        };
        ...
        return (
            <div className="approve-comment nc-theme-sp-detail-bgc">
                <div className="comment-sel">
                    {/* { index_013 审批附件 } */}
                    <NCTooltip overlay={json['index_016']} placement="bottomRight">
                    <span className="sel-item" onClick={(eve) => {
                            this.setState({
                                showUploader: true
                            });
                        }}>{json['index_013']}</span>
                    </NCTooltip>
                    <NCTooltip overlay={tips.copy} placement="bottomRight">
                        <span className="sel-item" onClick={this.openUserModal}>
                    ...
                    <span className="sel-item">
                        <NCTooltip overlay={tips.track} placement="bottomRight">
                            <span onClick={() => this.props.setAttr({isTrack: 'set'})} style={{marginRight: 5}}>{json['index_006']}</span> 
                        </NCTooltip>
                        <NCCheckbox 
                        type="checkbox"
