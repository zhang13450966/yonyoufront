 ...
 * @Last Modified time: 2019-05-24 17:24:23
 */
		...
		return (
			<div className={className}>
				{showAllCheck && <div className="user-transfer-list-header">
					...
					<span 
						fieldid={getSysFieldid('openlist')}
				...
				<div className={`${!this.state.openList && showAllCheck&& 'hide-list'} user-transfer-list-body`}>
					<ul className="user-transfer-list-content">
						{dataSource.map((item, index) => {
							...
							return <li 
									className="user-transfer-list-content-item nc-theme-copytransfer-list-item-hover-bgc"
									key={item.key}
								...
								{showCheck &&<NCCheckbox 
									...
									onClick={showCheck && this.itemClick.bind(this, item)}
								/>}
								<span className="nc-theme-common-font-c">{`${item.user_code?item.user_code.value:''} ${item.user_name?item.user_name.value:''} `}</span>
								{!showCheck &&<span 
									className="close-item iconfont icon icon-guanbi nc-theme-common-font-c"
