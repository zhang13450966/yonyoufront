...
import { ajax, toast, base, getMultiLang, pageTo,promptBox } from 'nc-lightapp-front';
...
export default class ApproveBtns extends Component {
        ...
        ajax({
            ...
            success: (res) => {
                ...
                if(data && data.content){
                    ...
                    toast({color: 'danger', content: this.state.json['011']}) // 011 操作失败
                }else{
    ...
    //执行批准
    approvePass2 = () => {
        const { billMessage, attrfiles,isTrack } = this.props;
        let data = {
            "from_terminal":"pc",
            billtype: billMessage.billtype,
            billid: billMessage.billid,
           ...
           // url: '/nccloud/workflow/approvalcenter/approveNoPassAction.do',
            ...
            success: (res) => {
                ...
                if (res.data && (res.data == "200" || res.data.code == "200")) {
                    ...
                    toast({ content: that.state.json['011'], color: 'danger' }); // 011 操作失败
                }
            }
        });
    }
    ...
    //执行指派
    approveAssgin =async (value, approveTransComment) => {
        let _this = this;
        let msg = _this.props.message;
        //智能审核失败
        if(msg&&msg.pointStatus&&msg.pointStatus=='4'){
          ...
          if(_this.props.hasOwnProperty('saveApprovePoint')){
           ...
           if(msg&&msg.pointStatus&&msg.pointStatus!=5){
              ...
              let data={
                pk_checkflow:pk_checkflow,
                check_note: approveTransComment || this.state.comment,
                checknote: approveTransComment || this.state.comment,
              }
              let devModule=this.props.devModule;
              let cadevUrl= getRealUrl(devModule,'workflow/approvalcenter/approvePassPointCheckAction.do');
              ajax({
                ...
                success: (res) => {
                    ...
                    if(res.data.haveNoPassPoint&&res.data.haveNoPassPoint=='true'){
                        promptBox({
                            ...
                            cancelBtnName: _this.state.json['10160501-000192'],  
                            hasCloseBtn:false,             //显示“X”按钮，默认不显示，不显示是false，显示是true
                            beSureBtnClick: () => {_this.beSureBtnClickApproveAssgin2(value) },   // 确定按钮点击调用函数,非必输
                            // cancelBtnClick: functionCancel,  // 取消按钮点击调用函数,非必输
                            // closeBtnClick:functionClose,    //关闭按钮点击调用函数，非必输
                            closeByClickBackDrop:false,    //点击遮罩关闭提示框，默认是false点击不关闭,点击关闭是true
                            // zIndex:200                     //遮罩的层级为zIndex，弹框默认为zIndex+1。默认为200，非比传项
    ...
    //审批提示窗确认执行
    confirmApprove = (result, comment) => {
        this.approveAssgin(result, comment);
    }
    ...
    render() {
        ...
        return <span className="approve-btns">
            <span className="btns">
                ...
                {!btns.includes('unapprove') && btns.length>0 && <NCButton fieldid="workflow-pass" colors="primary" onClick={this.onPass}>{btns[0]}</NCButton>}
                {/* 026 撤销 */}
                ...
                {btns.includes('reject') && <RejectBtn
                    ...
                    addParams={addParams}
                    isBillEditStatus={this.props.isBillEditStatus}
                    />}
                {/* 028 工作流程 */}
                {!this.props.hideApproveDetail && <NCButton fieldid="workflow-process" onClick={this.showWorkflow}>{json['028']}</NCButton>}
                {/* 改派按钮 */}
            ...
            {/* 加签弹框 */}
            <NCModal fieldid="addApprover" size="sm" className="form-modal addApprover" mask={true} visible={this.state.showAddApproveModal}>
                    <NCModal.Header>
                    ...
                    <NCModal.Body>
                        <div className="body-content">
                            <div className="content-inner">
                                <div className="label"><i style={{color: 'red', marginRight: 4}}>*</i>{json['046']}</div>{/* 国际化处理： 选择加签人*/}
                            ...
                            <div className="content-inner">
                                <div className="label">{json['049']}</div>{/* 国际化处理： 加签原因*/}
                                <div className="content">
                                    <NCTextArea fieldid="comment" style={{width: "100%"}} value={this.state.addApproveSuggestion}
